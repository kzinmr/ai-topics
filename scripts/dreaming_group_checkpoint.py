#!/usr/bin/env python3
"""Load grouped dreaming themes from the latest group cron output."""
...
def main() -> int:
    ...
    output_dir = hermes_home / "cron" / "output" / GROUP_JOB_ID
    ...
    if not files:
        print(json.dumps({
            "ok": False,
            "error": "dreaming-group output not found",
    ...
    latest_path = checkpoint_dir / "grouped_themes_latest.json"
    latest_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    payload["_group_checkpoint"] = {
        "ok": True,
        "output_path": str(latest_output),
