#!/usr/bin/env python3
"""Load the latest blog-triage output for wiki ingestion."""
...
def main() -> int:
    ...
    if not files:
        print(json.dumps({
            "ok": False,
            "error": "blog-triage output not found",
    ...
    if not payload:
        print(json.dumps({
            "ok": False,
            "error": "failed to parse JSON response from blog-triage output",
    ...
    if archive_script.exists():
        try:
            result = subprocess.run(
                [str(archive_script), "blog", "--keep-reference"],
                capture_output=True, text=True, timeout=30,
            )
            archive_data = json.loads(result.stdout) if result.stdout else {}
            payload["_archive"] = archive_data.get("blog", {"error": "no blog key"})
