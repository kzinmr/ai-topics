...
- **`hn_keyword_supplement.py` is NOT runnable as-is (VERIFIED 2026-09-21)**: the file in the skill's `scripts/` dir is a markdown doc whose front prose starts at line 1 (no frontmatter), and the actual Python lives in a fenced code block. `python3 hn_keyword_supplement.py` dies with `SyntaxError: invalid decimal literal` on line 4. Fix: extract the ```python block from the md file and `write_file` it as a clean `/tmp/hn_kw_run.py`, then run that. Do NOT `write_file` over the skill's copy with the extracted code — the deployed skill dir is git-managed in ai-topics and other jobs may have it modified.
- **Stale cron checkpoint JSONs**: `newsletter-triage/latest.json` and `triage/latest.json` can carry months-old timestamps (verified 2026-09-21: both were May/July while `newsletter/latest.json` had today's run_id). Always check the timestamp field inside each latest.json before treating it as today's data; use the top-level ingest checkpoints (`newsletter/latest.json` run_id, `blog_ingest/latest.json` date) for "what happened today". A `processed_count: 0` newsletter run means legitimately no newsletters, not a failed pipeline.
- **Direct keyword search for late-breaking AI stories (VERIFIED 2026-09-14)**: `hn_algolia_supplement.py`'s front-page tags-only scan missed this week's top AI stories (Bengio "Why are AI agents lying, cheating and coordinating?" 625pts, Claude 18+ 673pts, Garry Tan distillation 390pts, Tell HN OpenAI opt-in 482pts) because the front_page tag decays fast and point thresholds filter mid-size controversies. For trending-topics runs, ALSO run a second Algolia query with `query=<term>&tags=story&numericFilters=created_at_i%3E{cutoff},points%3E80` over ~8-10 AI keywords ("AI agent", "Claude", "OpenAI", "LLM", "open weights", "prompt injection", "coding agent", "MCP", "AI safety", "model release") over a 3-4 day window, dedup by objectID across the two passes. Reusable script: `scripts/hn_keyword_supplement.py` (copied from the 2026-09-14 run's /tmp/hn_fetch2.py). This is what surfaced the session's lead topics that the front-page scan alone would have missed.

- **Shorthand wikilinks resolve to suffix-matched files — grep before "fixing" (VERIFIED 2026-09-14)**: index.md entries like `[[entities/fable]]`, `[[entities/sero]]`, `[[entities/tencent]]` point at files with different names (`entities/fable` → `fable-com.md`? no — file is literally `fable.md` or a suffixed variant; `sero` → lives alongside `2026-09-13_github_0xsero_local-ai-registry.md` raw). Before creating a "missing" page or reporting a ghost entry, `find wiki/entities -name '*keyword*'` — the entity exists under a different suffix far more often than it is genuinely absent. Same rule as pitfall #3 extended to filename suffixes.
