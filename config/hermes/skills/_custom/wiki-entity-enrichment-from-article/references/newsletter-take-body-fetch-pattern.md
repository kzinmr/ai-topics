...
Validated 2026-08-16 (Flue 2 Latent Space interview + Superintel+ GLM-5.3 deepdive).
...
targets = {"slug1": "https://...", "slug2": "https://..."}
...
for name, ext in results.items():
    ...
    for i, p in enumerate(ext["article_paras"][:25]):
        print(f"[{i}] {p[:400]}")
...
|--------|--------------|------------|-------|
| substack custom domain (`www.latent.space/p/{slug}`, `open.substack.com/pub/...`) | 200 with browser UA | `<article>` tag present; JSON-LD `headline` works | Flue 2 interview: 42 paras. First para is chat UI noise (author / date / share buttons) — strip manually |
...
- **`git pull --rebase` fails with unstaged changes** when unrelated files are dirty
  (e.g. `config/hermes/skills/` modified by sibling processes / skill-drift). If `git push`
  then succeeds anyway (no remote divergence), no action needed. Always `git add wiki/`
  explicitly (never `git add .`) to avoid sweeping sibling changes into your commit.
