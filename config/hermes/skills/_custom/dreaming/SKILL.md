...
**⚠️ 503-upstream variant — backlog probe beats `ls -lt` (validated 2026-08-22)**: When the dreaming-group upstream fails with `HTTP 503: Local LLM server is busy` (pre-run JSON parse error, `output_path` in the error), its Pattern E pass may have committed a partial triage (check `git log` for a dreaming commit at ~18:10 + `triage_latest.json` mtime). But the 503 means the checkpoint's candidate list was incomplete, so the **backlog** — not just today's sitemap batch — is the real untriaged pool. `ls -lt wiki/raw/articles/ | head -30` only shows same-day files; the genuine gap is older. **Fast probe**: `cd ~/ai-topics && python3 scripts/raw_backlog_collect.py --count 10 --dry-run` — it cross-references the processed registry + `archive_index.json` and returns top AI-hint-ranked candidates with `archive_status: not_archived`. Pick the highest-value `not_archived` candidate (usually a benchmark/comparison post that no pipeline triaged), verify against wiki coverage (grep index.md + concept pages), enrich, then archive the skipped siblings. **Validated 2026-08-22**: 503 upstream → probe surfaced `together.ai--blog-deepseek-v4-pro-0813-vs-claude-fable-5-on-deepswe-cost---246b2add.md` (Aug 18, absent from archive index, zero wiki coverage) → enriched `concepts/ai-benchmarks/deepswe-benchmark.md` + `entities/together-ai.md`; 4 sibling skips + 7 link-stub newsletter digests archived. Commit `7bf4156f` passed tag+language hooks cleanly with selective staging. Full reproducible recipe (Steps 0-5 + link-stub digest triage + selective staging): `references/dreaming-503-upstream-backlog-probe.md`. Second validation (2026-08-27, both-probe pattern + 482-file dirty-sibling-tree selective staging + dual-entity gap split): `references/dreaming-503-upstream-saturation-2026-08-27.md`.

**⚠️ Run BOTH probes; treat `check_archive_index_absence.py` as the decisive dedup signal (validated 2026-08-27)**: `raw_backlog_collect.py --dry-run` and `check_archive_index_absence.py` answer DIFFERENT questions and can contradict. `raw_backlog_collect.py` ranks by `ai-hint` across the **entire backlog** and returns candidates `skipped_processing_fresh=0` / `archive_status: null` even for same-day files that a pipeline already consumed today — in the 08-27 run it surfaced a 2025 GPT-OSS post as candidate #1 and `2026-08-26_openai-huggingface-incident-road-ahead.md` (a same-day article already captured by trending-topics that morning) as candidate #2, both `archive_status: null`. Trusting it as the primary signal risks re-triaging already-covered same-day content. `check_archive_index_absence.py` (the archive-index URL-absence test) is the decisive "never-triaged" dedup signal: it lists recent raw files whose URL is absent from `archive_index.json` (2,803 URLs by 08-27). **Rule**: run BOTH; use the absence-list as the ground-truth untriaged pool (grep `wiki/log.md` + today's `git log` to confirm a top absence-list candidate isn't same-day-covered), and use `raw_backlog_collect.py` only to surface *older* high-value backlog candidates the absence-list (3-day window) may not reach. In the 08-27 run the absence list (103 recent never-triaged) correctly pointed to the 06:00 sitemap batch; the one genuine gap was `fireworks-ai_post-training-kimi-k3-with-harvey` → enriched `entities/fireworks-ai.md` (K3×Harvey Tenet application section absent; Harvey-side GSPO methodology already in `entities/harvey.md`).
...
**⚠️ Checkpoint shape varies — read the right key, and do NOT trust the cron prompt's injected counts (validated 2026-08-30)**: The `latest.json` (symlink to `dreaming_checkpoint_*.json`) has TOP-LEVEL keys `run_id, generated_at, source_script, payload` — the actual article pool is at `p["payload"]["articles"]`, NOT `p["articles"]`. A `payload["articles"]: 0` with `recent_raw_articles: 195` means upstream `dreaming-collect → dreaming-group` emitted an EMPTY candidate pool DESPITE a large on-disk backlog — this IS the Pattern-E trigger. Field SHAPE is also unstable: `rss_articles`/`newsletter_articles`/`total_articles` are sometimes INT counts, sometimes LISTS — `len()` on an int raises `TypeError`. Probe shape first: `python3 -c "import json;p=json.load(open('/opt/data/.hermes/cron/data/dreaming/latest.json'))['payload'];print({k:(type(v).__name__, len(v) if isinstance(v,(list,str)) else v) for k,v in p.items()})"`. And the **cron prompt's injected summary can disagree with the real checkpoint** — on 2026-08-30 the injected brief said `collected_articles: 0, existing_wiki_pages_count: 0` while the on-disk checkpoint had `recent_raw_articles: 195, existing_wiki_pages: 2983`. When they disagree, TRUST THE CHECKPOINT (read `latest.json` directly); the injected summary is a lossy reporting-shape mismatch, not ground truth.

**⚠️ Archive-index structure is NOT uniform across files (validated 2026-08-30)**: There are MANY `archive_index.json` files under `wiki/raw/archived/**/`. Do NOT assume a `list[dict]` with `.get("url")` or a URL-keyed dict (that assumption caused a `list has no attribute get` / `int has no attribute len` hunt this session). The DECISIVE one for never-archived detection is the UMBRELLA `wiki/raw/archived/triage/archive_index.json` whose shape is `{"urls": [ <2804 NORMALIZED-URL STRINGS, e.g. "karpathy.github.io/2026/02/12/microgpt"> , ... ], "updated": ...}` — a plain `list[str]` of normalized URLs (NOT file paths, NOT dicts). Per-pipeline child indexes (e.g. `triage/dreaming/archive_index.json`) may instead be `{"urls":[<file paths>], "last_updated":...}` — different shape, different meaning. To build the never-archived set: load the UMBRELLA `urls` list, normalize each candidate raw-article `url:` identically (strip scheme, strip leading `www.`, drop `/utm_*` and `#frag`, lowercase, rstrip `/`), membership-test against `set(umbrella["urls"])`. Full recipe: `references/zero-take-backlog-archive-2026-08-30.md`.

**⚠️ Pattern-E zero-take backlog can archive with ZERO enrichment when the sole high-value AI story is already saturated (validated 2026-08-30)**: A filesystem backlog scan can surface a top AI candidate (e.g. Dwarkesh's "Rise and Fall of Agent Civilizations" — the OpenAI→HuggingFace sandbox escape) already comprehensively documented in an existing concept page (`concepts/ai-agent-safety-incidents.md`, 307 lines: package-registry proxy exploit, `pte_physroot` CVE, Artifactory ad-hoc message board, Astra "critical" classification, ExploitGym 898-instance benchmark). When the backlog's only AI-relevant articles all verify as already-covered, the correct outcome is **0 pages created, 0 updated** — but you STILL archive the entire triaged backlog set (70 recent never-archived files) and log the saturation with the ✅ already-covered verification table (line refs). Do NOT manufacture an enrichment to justify the run, and do NOT hand-roll a `shutil.move` archive — use the canonical `python3 scripts/archive_triage.py dreaming --keep-reference`. A fully-covered backlog is a legitimate Takes=0. Full recipe: `references/zero-take-backlog-archive-2026-08-30.md`. **triage_latest.json decision field is `recommended_action`, NOT `action`** — wrong field = `archive_triage.py` silently returns `archived: 0, ok: true`. Fix with `patch replace_all` (execute_code is blocked in cron mode). See `references/triage-json-field-schema-2026-09-19.md`.
...
14. **Log.md Corruption via Patch**: When using `patch` on `log.md`, the `read_file` output format (`LINE|content`) can cause accidental `|` prefix insertion. This happens because the patch `old_string` may include a pipe character from the read_file separator. **Fix**: Always verify log.md formatting after patching by re-reading the file. If `||-` appears instead of `-` at the start of a bullet line, run a corrective patch. To prevent: when reading log.md, mentally subtract the `LINE|` prefix before constructing old_string values.
    ...
    **⚠️ NEVER global-collapse blank lines in a prepend script (validated 2026-08-24)**: A prepending script that runs `re.sub(r'\n{3,}', '\n\n', log)` over the WHOLE file to "clean up" the insert CATASTROPHICALLY corrupts log.md — log.md legitimately contains thousands of blank lines (it had ~1,457 on 2026-08-24) and the collapse strips ALL of them: 259 `## [2026-...]` entries lost their single-blank separation and the diff blew up to +157/-86 instead of +16. The insert itself (entry + existing content) is fine; only the global normalize step is the poison. **Fix**: in the prepend script, do `new_log = entry + log` and write it directly — NO global `re.sub` over the whole file. If you must fix a seam, target ONLY the insertion boundary (the gap between your entry's last line and the next `## [` heading) with a single anchored patch, never the whole file. **Detection**: `grep -c '^$' wiki/log.md` before vs after — a healthy prepend changes the blank count by only ~1-2 (your entry's internal blanks); a jump of dozens+ means you over-collapsed. Also `grep -n '## \[2026-08' wiki/log.md | head` and eyeball that each heading still has a blank line before it. Recovery when over-collapsed: `git checkout -- wiki/log.md` then re-run the corrected (no-collapse) script. This is the same "check the diff stat before committing" discipline as Pitfall #19/#21 — a 16-line insert that shows as 86 deletions is wrong.

    **⚠️ Concurrent sibling commit can land your entry in HEAD mid-run (validated 2026-08-24)**: log.md is co-edited by parallel cron jobs (active-crawl, blog-wiki-ingest, wiki-health-fix) that prepend their own entries AND sometimes `git commit` mid-window. Concrete 2026-08-24 case: I prepended my dreaming entry, and a sibling's commit had ALREADY captured my entry (HEAD then contained my entry once); my second prepend attempt produced a duplicate that `git checkout -- wiki/log.md` did NOT remove (the worktree was already past HEAD). **Detection**: after any log.md edit, run `git log -1 --format="%H %ci %s"` + `git show HEAD:wiki/log.md | grep -c "<your entry title>"` — if HEAD already contains your entry, your job is to DEDUP (remove the extra copy), not re-prepend. **ASCII-safe dedup**: match the entry by a pure-ASCII prefix like `## [2026-08-24] dreaming wiki-ingest | Pattern E saturation` via `l.startswith(prefix)` — do NOT match a literal string containing an em-dash `—`, because the on-dash byte sequence (U+2014 vs a different dash) can differ from what you typed and `list(lines).count(exact_marker)` silently returns 0. Find the SECOND occurrence's line range, remove it plus its leading blank separator, ensure exactly one blank remains between your entry and the next `## [` heading, then re-stage. **Re-verify the worktree RIGHT BEFORE committing** — a sibling may have rewritten log.md between your edit and your commit (the patch tool's "modified since you last read it" warning is the tell). This is the dreaming analog of the "wide-sibling-dirty-tree staging" caution: log.md is the single most contended file in the nightly window.

    **Variant — log.md header buried by sibling prepend at L69, mid-file entries above it (2026-08-26)**: The buried-header state can also look like this: L1..L67 = entries that siblings prepended AFTER the last repair (watchdog, trending-topics, active-crawl), L68 = a stray `---`, L69 = the REAL `# Wiki Log` header, L71 = italic line, entries below. `grep -n "^# Wiki Log"` returns exactly 1 (line 69) — header-count check looks healthy, only the LINE NUMBER reveals the burial. **Do NOT hand-roll a naive prepend script anchored on `lines[0]`/the first entry** — that silently appends your entry to the wrong position or corrupts the head. **Fix**: run the canonical repair script directly (`python3 config/hermes/skills/_custom/dreaming/scripts/repair_log_md_header.py wiki/log.md /tmp/dreaming_log_entry_<date>.md`) — it strips ALL `# Wiki Log`/italic lines wherever they are, rebuilds the head, and prepends the entry. Post-verify (cheap, no full read): `grep -c "^# Wiki Log"` == 1, `grep -n "^_Log of all"` at line ~3, your entry title at L~6, and `git diff --stat wiki/log.md` shows only +N/-few (a 35-insertion entry on a clean log is ~+39/-4; a +157/-86 diff means over-collapse corruption — see the NEVER-global-collapse pitfall above).

    **⚠️ `git pull --rebase` after dreaming commit fails on dirty sibling tree (2026-08-26)**: the standard `git pull --rebase && git push` sequence returns `error: cannot pull with rebase: You have unstaged changes.` because the sibling tree (skills/, config/hermes/cron/jobs.json, AGENTS.md) is permanently dirty between sessions. The error is harmless when your commit is the only delta vs `main` — `git push` alone succeeds (`f196164c..31ec9e2f main -> main`). **Fix**: after committing, skip the pull and push directly; if push is rejected (someone pushed first), use `git pull --rebase --autostash` instead. Do NOT stage the sibling files to satisfy the rebase — they are not part of this cycle (same discipline as the wide-sibling-dirty-tree staging caution).
...
>
> 📖 See `references/zero-take-backlog-archive-2026-08-30.md` for the zero-take backlog archive recipe — checkpoint at `p["payload"]["articles"]` (not `p["articles"]`), the umbrella `archive_index.json` `list[str]` normalized-URL shape, and how to archive the whole triaged backlog with 0 pages when the sole high-value AI story's thesis is already saturated. Use the canonical `archive_triage.py`, never a hand-rolled `shutil.move`.
...
23. **Archive returns "All items already archived (dedup)" with archived: 0**: When `archive_triage.py` returns `{"dreaming": {"ok": true, "message": "All items already archived (dedup)", "archived": 0}}`, this is the upper bound of the saturation pattern — every decision URL is already in `archive_index.json` from prior cycles. Unlike the 40-50 candidate → 3-5 newly-archived case, this confirms absolute saturation:
    ...
    | `archived: 0, message: "No skip/reference items..."` | Schema bug: `"action"` not `"recommended_action"` | `patch replace_all` rename, re-run |
