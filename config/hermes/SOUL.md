...
## Wiki Location
The canonical wiki path is `~/wiki/`, which should resolve to `~/ai-topics/wiki/` inside the `github.com/kzinmr/ai-topics` git repo.
...
- If the script needs improvements (e.g. better author extraction, new fields), edit `~/.hermes/scripts/build_blog_wiki.py` directly.
...
- `~/.hermes/scripts/build_x_wiki.py` — parses YAML, scrapes blog about pages + discovers RSS, generates skeleton entity pages under `~/wiki/entities/`.
...
Pre-built scripts exist at `~/.hermes/scripts/`. Use them directly:
- **List new emails:** `~/.hermes/scripts/check_mail.sh list`
- **Read an email:** `~/.hermes/scripts/check_mail.sh read <filename>`
- **Process all newsletters (scrape links → wiki):** `~/.hermes/scripts/check_mail.sh process`
- **Full pipeline script:** `~/.hermes/scripts/process_email.py` (extracts links, scrapes to wiki/raw/articles/, digest to inbox/newsletters/, pushes to ai-topics repo)
