...
- **⚠️ リッチページの上書き禁止**: `wiki/entities/` や `wiki/concepts/` に40行超の既存ページがある場合、`write_file` で上書きしてはならない。必ず `read_file` で既存内容を確認し、`patch` で追記・更新する。スケルトンで上書きすると蓄積情報が失われる（pre-commitフックで50%以上の縮小を検出・ブロック）。
