...
series_part: "前編（全2回）"
series_next: blog/2026-06-03_hermes_agent-as-second-brain-and-the-harness-investment.md
...
> **本稿は2部構成の前編（実装編）だ。** 「どう作り、どう回すか」を手を動かすレベルで扱う。一歩引いた地図——ハーネスとは何への投資なのか、AWSへのホスティング、組織への導入——は後編「[Hermes Agent を「自動化された第二の脳」として運用する（後編）](blog/2026-06-03_hermes_agent-as-second-brain-and-the-harness-investment.md)」で扱う。
...
**▶ 後編へ続く**：ここまでが「どう作り、どう回すか」だった。後編「[Hermes Agent を「自動化された第二の脳」として運用する（後編）— ナレッジ管理から、ハーネスという投資対象まで](blog/2026-06-03_hermes_agent-as-second-brain-and-the-harness-investment.md)」では、この実装を一歩引いた地図——ハーネスという投資対象、Open Harness と Framework の役割分担、$5 VPS から AWS Bedrock AgentCore までのホスティング、そして個人の脳から組織のコントロールプレーンへ——の上に置き直す。
