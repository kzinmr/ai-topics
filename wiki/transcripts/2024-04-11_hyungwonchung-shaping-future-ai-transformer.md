---
title: "Shaping the Future of AI from the History of Transformer — Hyung Won Chung (Stanford CS25)"
author: Hyung Won Chung (@hwchung27)
source: YouTube / Google Slides
url: https://www.youtube.com/watch?v=orDKvo8h71o
slides: https://docs.google.com/presentation/d/1u05yQQaw4QXLVYGLI6o3YoFHv6eC3YN8GvWD8JMumpE
event: Stanford CS25 — Transformers United, Spring 2024
duration: "36:30"
type: transcript
tags: [scaling-hypothesis, bitter-lesson, transformer-architecture, transcript]
---

# Shaping the Future of AI from the History of Transformer — Transcript

## Video Info

- **Published**: 2024-04-11
- **Duration**: 36:30
- **Speaker**: Hyung Won Chung (@hwchung27)
- **Event**: Stanford CS25 — Transformers United, Spring 2024
- **Full transcript**: `transcripts/2024-04-11_hyungwonchung-shaping-future-ai-transformer.en.vtt`
- **Companion article**: [[raw/articles/2024-04-11_hyungwonchung-shaping-future-ai-transformer]]

## Talk Overview

Stanford CS25 lecture on the scaling hypothesis and how the history of the Transformer architecture informs the future of AI. Covers encoder-decoder evolution, the bitter lesson, and implications for model design.
