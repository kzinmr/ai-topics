...
- 🆕 `entities/tambo.md` Created (L2) — open-source Generative UI ツールキット for React。Michael Magán & Michael Milstead (Seattle, 2024設立)。Zapier/Rocket Money/Solink 採用。SCHEMA.md `generative-ui` タグAdded。
- 🆕 `entities/buildy.md` Created (L2) — AIエージェント パーソナルWebアプリ 構築・永続化するプラットフォーム。ES module POST → 公開URL + KVストレージ。MCP経由 with 全エージェント対応。llms.txt/llms-full.txt with AI向けドキュメント完備。
- `SCHEMA.md`: Products カテゴリ `generative-ui` タグAdded
...
- **New:** `wiki/raw/articles/2026-04-17_parallel_automated-competitor-analysis.md` — How to automate competitor analysis with AI agents: Discover → Extract → Monitor パイプライン
...
- **New:** `wiki/raw/articles/2025-10-08_postman-state-of-api-2025-report.md` — Postman第7回年次APIレポート。5,700人以上's 開発者・アーキテクト・エグゼクティブ対象。AI-APIギャップ（89% AI使用、24%'s み AIエージェント向けAPI設計）、MCP認知度（70%） and 採用（10%）'s ギャップ、API収益化（65%'s 組織 APIから収益） 分析。
- **New:** `wiki/concepts/ai-ready-apis.md` — AI-Ready APIsconceptpage。AIエージェント API消費者 and して想定した設計原則：機械可読スキーマ、エージェント認識セキュリティ、コントラクトテスト。API-First and 収益's 相関分析 含む。
- **Updated:** `wiki/concepts/mcp.md` — MCPpage Postman調査's 認知度・採用ギャップ data （70%認知/10%日常使用） and Postmanプラットフォーム's AIトラフィック分析（OpenAI 56%, Llama 6.9x成長） Added。
...
- **Deduplication:** `entities/nvidia-nemotron-3.md` Deleted（既存's `entities/nvidia-nemotron-3-nano-omni.md` and 重複）

## [2026-05-11] manual | Garry Tan's Meta-Meta-Prompting X Article ingested

- **New:** `wiki/raw/articles/2026-05-09_garrytan_meta-meta-prompting.md` — Garry Tan (YC CEO) 's X Article。ABMedia 's 中国語要約から再構成。Fat Skills, Fat Code, Thin Harness、Skillify、100+ skills + 100K pages personal AI OS。
...
- **Sources:** ABMedia (https://abmedia.io/garry-tan-meta-meta-prompting), xurl メタ data, web_extract プレビュー

## [2026-05-11] blog-wiki-ingest | Meta employee keystroke data collection (ATA/MCI)

- **Updated:** `wiki/entities/meta.md` — Added Agent Transformation Accelerator (ATA) / Model Capability Initiative (MCI) section.2026年4月より米国従業員's マウス操作・キーストローク・スクリーンショット AIエージェント訓練用 収集。Andrew Bosworth CTO 's ビジョン、プライバシー懸念、組織的文脈（10%レイオフ） 含む。
- **Sources:** Reuters (Katie Paul, Jeff Horwitz), 2026-04-21
- **全14件中:** take=0, skip=13（同日早朝blog-ingest with 既処理5件、AI非関連8件）, reference=1（Meta記事→entities/meta.mdUpdated）

## [2026-05-11] newsletter-wiki-ingest | The Signal + Superintel — Colossus deal, Anthropic/OpenAI updates, Jim Fan + AI governance

- **Updated:** `concepts/xai-anthropic-colossus-deal.md` — Colossus 2 詳細（1.5GW/550K GPUs）、経済分析（$5B→$15B）、Anthropic 利用者へ's 直接恩恵（レート制限2倍/ピーク時制限撤廃）、軌道コンピュート計画 's 4 section Added
- **Updated:** `entities/anthropic.md` — Code w/ Claude 2026 新発表: M365 GA（Excel/PPT/Word GA, Outlook beta）、Dreaming research preview、金融テンプレート、$50B調達/$1T評価額、計算資源パートナーシップ全体像（SpaceXAI/Amazon/Google/Microsoft/Fluidstack） Added
- **Updated:** `entities/openai.md` — May 2026 新製品: GPT-Realtime-2（音声推論/70+言語翻訳/ストリーミング文字起こし）、Codex ブラウザプラグイン（Chrome/macOS/Windows）、MRC open-source化（OCP経由、NVIDIA/AMD/Broadcom/Intel/Microsoft共同開発） Added
- **Updated:** `entities/jim-fan.md` — Sequoia講演（May 2026）: Dexterity scaling laws、2.1万時間人間視点映像、ロボット data 0.1%未満、Internet-scale data 不足課題 Added
- **Updated:** `concepts/agentic-ai-governance.md` — 「Trustworthy AI = Delegated Agency」フレームワーク: Control→Delegationパラダイムシフト、5次元対比表、業界動向 and 's 接続（Anthropic/OpenAI/StrongDM）、Open Questions Added
...
- **Updated:** `wiki/entities/seangoedecke-com.md` — "The left-wing case for AI" 記事Added。Disability rights, healthcare access, class mobility, educational equity, techno-utopianism 's 5つ's 論点。
- **Created:** `wiki/concepts/enterprise-ai-scaling-patterns.md` — OpenAI 企業インタビュー 基づく5つ's パターン: Culture Before Tooling, Governance as Enabler, Ownership Over Consumption, Evaluation Rigor, Protecting Judgment Work.
- **Updated:** `wiki/entities/simon-willison.md` — 2026-05-10 記事Added (NYT AI-generated quote incident, Andrew Quinn wheel-reinvention quote)
...
- **eval-tools-comparison consolidation:** `concepts/ai-eval-tools-comparison.md` + `concepts/eval-tools-comparison.md` (両方 stub) → `comparisons/eval-tools-comparison.md` moved_from Added、Reference4件 fixed 
- **hermes-vs-openclaw consolidation:** `concepts/openclaw-vs-hermes-architecture-comparison.md` + `concepts/hermes-agent-vs-openclaw-architecture-comparison.md` (両方 stub) → `comparisons/hermes-vs-openclaw-architecture.md` moved_from Added
- **Deleted:** `concepts/local-llm-models-comparison-open-weights-moe-hardware-vram.md` (stub 、異常なタイトル)
- **Deleted:** `concepts/dspy-comparisons.md` (低品質、Reference5件→ `concepts/dspy.md` 振替)
...
- **Consolidation:** `concepts/agent-harness-comparison.md` (404行, 本文) + `comparisons/coding-agent-harnesses.md` (22行, リダイレクト) → `comparisons/agent-harnesses.md`
- **Deleted:** 元's 2ファイル Deleted
- **Reference fixes:** `[[concepts/agent-harness-comparison]]` 全Reference → `[[comparisons/agent-harnesses]]` (entities x8, concepts x2, index.md)
- **Reference fixes:** `[[comparisons/coding-agent-harnesses]]` → `[[comparisons/agent-harnesses]]` (index.md)
- **Updated:** frontmatter `moved_from` Added、`type: comparison`、内部's 循環Reference 除去
...
- **Removed:** `wiki/concepts/comparisons/` directory Deleted（空 directory ）
- **Updated:** frontmatter `moved_from` Added、related pages main concept page へ's リンクAdded
...
- **Created:** `wiki/concepts/rao-recursive-agent-optimization.md` — RAO (Recursive Agent Optimization): CMU × Amazon AGI Labs end-to-end RL training method for recursive agents.Qwen3-4B-Instruct 75ステップ訓練するだけ with deep research タスク with 単一エージェント比 +16% SR。
- **Created:** `wiki/entities/apurva-gandhi.md` — Apurva Gandhi: CMU PhD student (advised by Graham Neubig). Former Microsoft Copilot infrastructure design.RAO、Go-Browse、SkillWeaver 's 著者。
- **Saved:** `wiki/raw/papers/2026-05-07_2605.06639_recursive-agent-optimization.md` — RAO 論文 (arXiv:2605.06639)
- **Saved:** `wiki/raw/articles/2026-05-08_x-thread_apurva-gandhi-rao-recursive-agent-optimization.md` — Apurva Gandhi 's X スレッド (1/10〜10/10)
...
- **Created:** `wiki/concepts/baudrillard-and-ai.md` — Concept page applying Baudrillard's 'Simulacra and Simulation' framework (4 stages of the image, hyperreality, precedence of simulacra) to AI phenomena in the LLM era.AI: 模倣's 失敗 with : なくシミュラークル's 完成形 with ある and いう視点。
- **Saved:** `wiki/raw/articles/2025-05-10_ian-yang-welcome-to-the-token-of-the-real.md` — Ian Yang's LLM記号論分析
- **References:** filfre.net（直接's ボードリヤール言及）, onehappyfellow (Simulacrum of Knowledge Work), 関連concept: waluigi-effect, linguistic-vertigo, illusion-of-thinking, societal-shadow, cognitive-debt
...
- **Reference path fixes:** `wiki/concepts/gepa.md`, `wiki/concepts/rlms.md`, `wiki/concepts/recursive-language-models.md` 's sources canonical path 統一
- **Dedup mechanism:** `scripts/papers_index.py` 導入（`.papers_index.json` with arXiv ID → ファイル名マッピング。`--check` / `--add` / `--build`）
- **Naming convention codified:** `raw-article-filename-policy` skill `{YYYY-MM-DD}_{arxiv-id}_{short-title}.md` ルールAdded
