...
- 🆕 `entities/tambo.md` Created (L2) — open-source Generative UI ツールキット for React。Michael Magán & Michael Milstead (Seattle, 2024設立)。Zapier/Rocket Money/Solink 採用。SCHEMA.md に `generative-ui` タグAdded。
- 🆕 `entities/buildy.md` Created (L2) — AIエージェントがパーソナルWebアプリを構築・永続化するプラットフォーム。ES module POST → 公開URL + KVストレージ。MCP経由で全エージェント対応。llms.txt/llms-full.txt でAI向けドキュメント完備。
- `SCHEMA.md`: Products カテゴリに `generative-ui` タグAdded
...
- **Updated:** `wiki/entities/parallel-web-systems.md` — Key ArticlesにAdded
...
- **New:** `wiki/raw/articles/2025-10-08_postman-state-of-api-2025-report.md` — Postman第7回年次APIレポート。5,700人以上の開発者・アーキテクト・エグゼクティブ対象。AI-APIギャップ（89%がAI使用、24%のみがAIエージェント向けAPI設計）、MCP認知度（70%）と採用（10%）のギャップ、API収益化（65%の組織がAPIから収益）を分析。
- **New:** `wiki/concepts/ai-ready-apis.md` — AI-Ready APIsconceptpage。AIエージェントをAPI消費者として想定した設計原則：機械可読スキーマ、エージェント認識セキュリティ、コントラクトテスト。API-Firstと収益の相関分析を含む。
- **Updated:** `wiki/concepts/mcp.md` — MCPpageにPostman調査の認知度・採用ギャップデータ（70%認知/10%日常使用）とPostmanプラットフォームのAIトラフィック分析（OpenAI 56%, Llama 6.9x成長）をAdded。
...
- **Deduplication:** `entities/nvidia-nemotron-3.md` Deleted（既存の `entities/nvidia-nemotron-3-nano-omni.md` と重複）

## [2026-05-11] manual | Garry Tan's Meta-Meta-Prompting X Article ingested

- **New:** `wiki/raw/articles/2026-05-09_garrytan_meta-meta-prompting.md` — Garry Tan (YC CEO) の X Article。ABMedia の中国語要約から再構成。Fat Skills, Fat Code, Thin Harness、Skillify、100+ skills + 100K pages personal AI OS。
...
- **Sources:** ABMedia (https://abmedia.io/garry-tan-meta-meta-prompting), xurl メタデータ, web_extract プレビュー

## [2026-05-11] blog-wiki-ingest | Meta employee keystroke data collection (ATA/MCI)

- **Updated:** `wiki/entities/meta.md` — Added Agent Transformation Accelerator (ATA) / Model Capability Initiative (MCI) section.2026年4月より米国従業員のマウス操作・キーストローク・スクリーンショットをAIエージェント訓練用に収集。Andrew Bosworth CTO のビジョン、プライバシー懸念、組織的文脈（10%レイオフ）を含む。
- **Sources:** Reuters (Katie Paul, Jeff Horwitz), 2026-04-21
- **全14件中:** take=0, skip=13（同日早朝blog-ingestで既処理5件、AI非関連8件）, reference=1（Meta記事→entities/meta.mdUpdated）

## [2026-05-11] newsletter-wiki-ingest | The Signal + Superintel — Colossus deal, Anthropic/OpenAI updates, Jim Fan + AI governance

- **Updated:** `concepts/xai-anthropic-colossus-deal.md` — Colossus 2 詳細（1.5GW/550K GPUs）、経済分析（$5B→$15B）、Anthropic 利用者への直接恩恵（レート制限2倍/ピーク時制限撤廃）、軌道コンピュート計画 の4セクションAdded
- **Updated:** `entities/anthropic.md` — Code w/ Claude 2026 新発表: M365 GA（Excel/PPT/Word GA, Outlook beta）、Dreaming research preview、金融テンプレート、$50B調達/$1T評価額、計算資源パートナーシップ全体像（SpaceXAI/Amazon/Google/Microsoft/Fluidstack）をAdded
- **Updated:** `entities/openai.md` — May 2026 新製品: GPT-Realtime-2（音声推論/70+言語翻訳/ストリーミング文字起こし）、Codex ブラウザプラグイン（Chrome/macOS/Windows）、MRC open-source化（OCP経由、NVIDIA/AMD/Broadcom/Intel/Microsoft共同開発）をAdded
- **Updated:** `entities/jim-fan.md` — Sequoia講演（May 2026）: Dexterity scaling laws、2.1万時間人間視点映像、ロボットデータ0.1%未満、Internet-scaleデータ不足課題 をAdded
- **Updated:** `concepts/agentic-ai-governance.md` — 「Trustworthy AI = Delegated Agency」フレームワーク: Control→Delegationパラダイムシフト、5次元対比表、業界動向との接続（Anthropic/OpenAI/StrongDM）、Open Questions をAdded
...
- **Updated:** `wiki/entities/seangoedecke-com.md` — "The left-wing case for AI" 記事Added。Disability rights, healthcare access, class mobility, educational equity, techno-utopianism の5つの論点。
- **Created:** `wiki/concepts/enterprise-ai-scaling-patterns.md` — OpenAI 企業インタビューに基づく5つのパターン: Culture Before Tooling, Governance as Enabler, Ownership Over Consumption, Evaluation Rigor, Protecting Judgment Work.
...
- **eval-tools-comparison consolidation:** `concepts/ai-eval-tools-comparison.md` + `concepts/eval-tools-comparison.md` (両方スタブ) → `comparisons/eval-tools-comparison.md` に moved_from Added、Reference4件修正
- **hermes-vs-openclaw consolidation:** `concepts/openclaw-vs-hermes-architecture-comparison.md` + `concepts/hermes-agent-vs-openclaw-architecture-comparison.md` (両方スタブ) → `comparisons/hermes-vs-openclaw-architecture.md` に moved_from Added
- **Deleted:** `concepts/local-llm-models-comparison-open-weights-moe-hardware-vram.md` (スタブ、異常なタイトル)
- **Deleted:** `concepts/dspy-comparisons.md` (低品質、Reference5件→ `concepts/dspy.md` に振替)
...
- **Updated:** frontmatter に `moved_from` Added、`type: comparison`、内部の循環Referenceを除去
...
- **Removed:** `wiki/concepts/comparisons/` ディレクトリをDeleted（空ディレクトリ）
- **Updated:** フロントマターに `moved_from` Added、related pages に main concept page へのリンクAdded
...
- **Created:** `wiki/concepts/rao-recursive-agent-optimization.md` — RAO (Recursive Agent Optimization): CMU × Amazon AGI Labs end-to-end RL training method for recursive agents.Qwen3-4B-Instruct を75ステップ訓練するだけで deep research タスクで単一エージェント比 +16% SR。
- **Created:** `wiki/entities/apurva-gandhi.md` — Apurva Gandhi: CMU PhD student (advised by Graham Neubig). Former Microsoft Copilot infrastructure design.RAO、Go-Browse、SkillWeaver の著者。
- **Saved:** `wiki/raw/papers/2026-05-07_2605.06639_recursive-agent-optimization.md` — RAO 論文 (arXiv:2605.06639)
- **Saved:** `wiki/raw/articles/2026-05-08_x-thread_apurva-gandhi-rao-recursive-agent-optimization.md` — Apurva Gandhi の X スレッド (1/10〜10/10)
...
- **Created:** `wiki/concepts/baudrillard-and-ai.md` — Concept page applying Baudrillard's 'Simulacra and Simulation' framework (4 stages of the image, hyperreality, precedence of simulacra) to AI phenomena in the LLM era.AIは模倣の失敗ではなくシミュラークルの完成形であるという視点。
- **Saved:** `wiki/raw/articles/2025-05-10_ian-yang-welcome-to-the-token-of-the-real.md` — Ian YangのLLM記号論分析
- **References:** filfre.net（直接のボードリヤール言及）, onehappyfellow (Simulacrum of Knowledge Work), 関連concept: waluigi-effect, linguistic-vertigo, illusion-of-thinking, societal-shadow, cognitive-debt
- **Updated:** `wiki/index.md` — Concepts セクションに baudrillard-and-ai をAdded、カウントUpdated
...
- **Reference path fixes:** `wiki/concepts/gepa.md`, `wiki/concepts/rlms.md`, `wiki/concepts/recursive-language-models.md` の sources を canonical path に統一
- **Dedup mechanism:** `scripts/papers_index.py` 導入（`.papers_index.json` で arXiv ID → ファイル名マッピング。`--check` / `--add` / `--build`）
- **Naming convention codified:** `raw-article-filename-policy` skill に `{YYYY-MM-DD}_{arxiv-id}_{short-title}.md` ルールAdded
