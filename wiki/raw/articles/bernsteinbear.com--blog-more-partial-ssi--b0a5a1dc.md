...
url: "https://bernsteinbear.com/blog/more-partial-ssi/?utm_source=rss"
fetched_at: 2026-08-14T10:21:53.403118+00:00
...
Source: https://bernsteinbear.com/blog/more-partial-ssi/?utm_source=rss

After reading
Chris Fallin’s aegraph
post
, new ZJIT contributor dak2
landed a block-local version of the
canonicalize
function in
#16828
.
...
v1
. This is important
because a later constant-folding pass can observe that the input
v1
of
GuardType
v1, Int
is
already an Int
and can therefore replace all uses of
v2
with
v1
and delete the guard.
Because in this local version we make a new
rewrite_map
for each block, we
...
maybe-easier-to-verify thing as a correctness oracle. So I did
the
silly slow thing in
#17766
.
...
=
rewrite_map
(Which you may or may not notice looks a lot like
Maxine’s GVN
implementation
.
...
inserted
RefineType
in SSA construction so that we can infer things about the
...
bb2:
  ...
In this more general case, we stil want
bb1
to know that
v0
is
...
CBool[false]
in
its branch (and in blocks dominated by
bb1
and
bb2
).
Well, this is another thing we can do in
canonicalize
!
All we need to do is at the beginning of each block B:
Check if B has one incoming control edge E
Check if the terminator T for E.block is a conditional branch
If T.iftrue == B, seed the
rewrite_map
with
T.cond => Const(CBool[true])
If T.iffalse == B, seed the
rewrite_map
with
T.cond => Const(CBool[false])
If you plan on running
canonicalize
multiple times, you may end up generating
...
Well, I admit I was looking at the
30k_ifelse
benchmark on ruby-bench and
...
bb2:
  ...
  CondBranch v0, ...
and it felt a little silly that
bb1
and
bb2
...
about
v0
by being branch targets. This couple-line change managed to collapse
