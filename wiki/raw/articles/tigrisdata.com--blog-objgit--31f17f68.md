...
fetched_at: 2026-06-25T07:01:41.210448+00:00
source: "xeiaso.net"
tags: [blog, raw]
---
...
README.md to it. The tree for the
.git
...
bits
never change
once they’ve been committed. Git objects are a great fit for
...
latest commit. These are
tiny
files though, which means that Tigris can handle
...
a git server
without
storing everything in the local filesystem, you have to
...
If you link into git’s guts with
libgit
, you inherit the “when things go
bad,
die()
” behaviour and your app now suddenly starts crashing at random.
This is not good for uptime.
If you try to use
libgit2
(the rewrite-that’s-actually-a-library), you have
...
everything else Just Working™ is perfectly logical. Git’s on-disk format
is
...
answer is to create exactly that intermediate state:
CopyObject
to the new
place and
DeleteObject
...
bucket 1:1. Objgit’s implementation of
Rename
is trivial:
copySource
:=
...
temporary file, it creates that temporary file and then
immediately
starts
...
the filesystem as trying to push
gcc.git
...
golang/go
repository to objgit to see how long
it would take. It did work, but it took
forever
. Using the prometheus metrics
I mentioned before, I saw that it was making biblical amounts of
HeadObject
...
syscalls resolve in
microseconds
, not the tens of milliseconds it takes to get
...
loose objects
when pushing it. Each loose object write was costing two round
trips:
stat()
to check if a file exists and then
open()
/
write()
to
...
reads; but this is a firehose of
writes
to 100,000 paths that probably have
...
library's fast path stores an incoming pack whole through its
PackfileWriter
,
by copying from the connection until
io.EOF
. Over HTTP that's fine: the
request body ends, EOF arrives, everyone goes home. Over
git://
and SSH, the
...
participants. The original workaround was to hide the
PackfileWriter
...
objects and a 200KiB packfile made over 8,500
GetObject
...
probes for a loose object at
objects/<xx>/<rest-of-hash>
. objgit keeps packs
...
actually useful and resulted in only one
ListObjectsV2
call instead of
...
point, Git
will
find it and the metrics will show you where that problem is.
...
kefka
sandbox with a
checkout of the git repository at the commit you just pushed mounted at
/src
and mutable temporary files at
/tmp
. It gets coreutils and nothing else. No
host filesystem, no network, no arbitrary binaries. Output streams back into the
pusher as
remote:
lines just like when you
git push heroku main
. Eventually
I want to make custom commands to allow you to deploy
Tekton
pipeline changes and kick off CI jobs that way,
...
that have
nothing
to do with git trees, object storage, or anything else and
...
that this doesn’t have load-bearing dependencies on
/usr/bin/git
would make it
