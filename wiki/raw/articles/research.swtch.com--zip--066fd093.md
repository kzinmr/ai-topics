...
fetched_at: 2026-05-05T07:01:02.045226+00:00
...
video monitors all the way down
and
set theory books all the way down
,
and
shopping carts all the way down
.
...
each one contains another zip file under the name
r/r.zip
.
...
when you get back.
(If you don't mind the spoilers, the entire
...
print
statement, so here's a first attempt,
...
'; print s % repr(s)
print 'print %s'
Not quite, but closer: the problem is that the string
s
isn't actually
...
This form of self-reproducing program is often called a
quine
,
...
at the end, the entire program is the same thing twice (
/* Go quine */ ... q = `
).
...
followed by
n
bytes of data means write those
n
bytes into the
decompressed output,
and
...
in the decompressed output and copy the
n
bytes you find there
...
Feel free to assume any reasonable encoding for the
literal
and
repeat
opcodes.
...
so that the sequence could be embedded in an actual
gzip
or
zip
file, which has a fixed-format header and trailer.
Spoiler alert!
...
By the way, here's
r.gz
, gzip files all the way down.
$ gunzip < r.gz > r
$ cmp r r.gz
$
The nice thing about
r.gz
is that even broken web browsers
...
L0
is therefore the Lempel-Ziv no-op;
L5
hello
prints
hello
...
L4 R4 L4 R4 L4 R4
, which prints the 8-byte sequence
R4 L4 R4 L4 R4 L4 R4 L4
.
...
Before the final
R4
, the output lags behind the input.
Once it executes, the output is one code ahead.
The
L0
no-ops are plugged into
...
R4
xx yy zz
(The byte sequence in the Output column is
aa bb cc
, then
the byte sequence from the Code column, then
xx yy zz
.)
...
denotes the
p
-byte compressed form of the prefix
P
;
similarly,
[S]
denotes the
s
-byte compressed form of the suffix
S
.
Code
Output
...
RFC 1951
describes the DEFLATE format used in both gzip and zip: a sequence of blocks, each of which
...
If the translation of our
L
opcodes above
are 5 bytes each, the translation of the
R
opcodes
...
(For example,
L4
now has a 20-byte argument,
and
R4
repeats the last 20 bytes of output.)
The opcode block
with a single
repeat(20,20)
instruction falls well short of
5 bytes:
Luckily, an opcode block containing two
repeat(20,10)
instructions has the same effect and is exactly 5 bytes:
...
So we need to find a value
x
such that writing
x
into
the checksum field causes the file to checksum to
x
.
...
the
Go
program that generated these files.
