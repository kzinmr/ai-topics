...
url: "https://jyn.dev/buffer-overflows-and-stacks-and-assembly-oh-my/"
fetched_at: 2026-04-28T07:02:51.884522+00:00
source: "jyn.dev"
tags: [blog, raw]
---
...
Source: https://jyn.dev/buffer-overflows-and-stacks-and-assembly-oh-my/
...
If you want to try it yourself, the binary is
here
,
...
But wait, how do we get the binary? It turns out on linux there's a
proc
filesystem
which
looks
like it has a bunch of files but really is a bunch of kernel info about
...
... more binary follows ...
/proc
is the start of the filesystem,
self
is the directory for the currently running process
(in this case the vulnerable program), and
exe
is the executable file as it was
...
Well that's a lot to look at. There's a function called
primary_login
...
secondary_login
looks promising, what's that look like?
...
There's a few things of note here. First is the
command_line
function,
which looks promising. Next is
_ZL4FLAG
, which seems useful. What's there?
(gdb) printf "%s\n", 0x41414a40
�����������������������������������/bin/sh
That's not helpful, it's binary data. And why does it have a shell at the end there?
And finally, how do we get to
command_line
...
(compare) and a
jae
(jump above equal) to +144. At 144, it compares
-0x10(%rbp) to 0x23, or 35 in decimal.
That's funny, the number of
�
characters in the data above is 35.
...
doing funny things with
xor
. If we xor the binary at
_ZL4FLAG
with
0xc7
, we get back the flag. GDB and python will both be helpful here.
...
print/x
means print as hexidecimal instead of decimal or binary.
''.join(hexes.split())
is a hacky way of removing whitespace.
The
replace
calls are just getting rid of formatting we don't care about.
re.findall('..', chars)
splits the strings into 2-character sequences.
...
Side note: originally this was even more hacky - I used gdb's
<address>@n
syntax
