...
tags: [distributed-systems, inference, p2p, edge-computing, model]
author: Rae McKelvey
---

# Mesh LLM: distributed AI computing on iroh

**Source:** [iroh.computer/blog/mesh-llm](https://www.iroh.computer/blog/mesh-llm)
...
The HN discussion (344 points, posted 2026-07-11) surfaced several key topics:

**Performance:** Qwen 235B A22B MoE achieved 16 tok/s across 2 nodes in split mode. The Skippy engine is built as a patch queue on top of llama.cpp, allowing access to internal activations and tensor filtering on model load. GLM 5.2 reached ~10 tok/s on a home lab setup (2 Mac Studios) with simulated 5ms latency.

**Latency Analysis:** Network latency is the primary bottleneck for token generation in split mode — not bandwidth, since only activation vectors (kilobytes) need to cross the network per token. On a 4-node split with 1ms latency per hop, theoretical max is ~30 tok/s. This makes split mode practical for LAN/metro-latency networks but slow over global WAN. Prefill/prompt processing doesn't suffer from this latency penalty since it can be parallelized.

**Comparison with exo:** Mesh LLM supports all platforms (not Mac-only like exo), uses iroh for P2P networking, and has built-in model splitting. Detailed comparison at meshllm.cloud/docs/pages/exo-comparison/.

**Privacy/Security:** Payloads are encrypted in transit (QUIC) but visible in plaintext at each node performing inference. No E2E encryption for prompts/responses. No incentive/reward mechanism for public mesh contributors yet — public mesh is currently open with no restrictions. Primary use case is private meshes with trusted peers.

**Fault Tolerance:** If a node drops during split inference, topology is recalculated and the request is automatically retried.

**Related Projects:** cocompute.ai, AI Horde, exo, query.mt, darkbloom.
