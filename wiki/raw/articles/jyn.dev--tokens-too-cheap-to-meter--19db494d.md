...
fetched_at: 2026-09-23T10:01:02.338967+00:00
source: "jyn.dev"
tags: [blog, raw]
---
...
Starting very soon, we are likely to see
quality
and
access
become the limiting factor to AI  use, not sheer number of tokens.
...
Generally, models intended to be run locally will be much smaller, such as
Muse Glimmer
or
Qwen3 Coder
.
...
In the graph below (
source
), the X-axis is time and the Y-axis is power efficiency of the GPU itself.
...
The cost per
token
of models is not consistently going down, at least not for the smartest ("frontier") models.
But the cost per
task
is.
...
pareto frontier
" of cost/task at present.
A pareto frontier shows the best
tradeoff
you can get, not just the best in a single category.
...
Models below the dotted line are basically not worth considering.
Now, look at this chart showing the frontier at the start, middle, and end of
2025
:
The chart shows models are getting smarter
and
cheaper on a per-task basis over 2025.
...
vLLM
vLLM
is an open-source inference engine and it's getting more efficient over time.
In the graph below (
source
), the Y-axis is Joules/token, the X-axis is batch size (roughly: "how many inputs are processed in parallel?"), and the blue/red lines are different software versions.
...
considering vLLM alone, and the performance gains for v2 ➝ v3 are roughly proportional to the energy efficiency improvement we have better numbers for.
NVIDIA
This isn't isolated to a single software package.
NVIDIA is showing up to 50% efficiency improvements on their
MLPerf
stack from 2.0 to 2.1:
Intel
This isn't isolated to old benchmarks.
Intel
recently showed
a 2.4x throughput increase solely by improving MLPerf between 6.0 and 6.1.
...
In the graph below, a model can be
7x smaller
(6B ➝ 0.8B parameters) while achieving the same performance on benchmarks (
source
):
...
mlx-flash
which swap layers into memory on-demand, but they only make these
possible
to run, not fast.
...
and you can't buy it because
all the AI companies bought it first
.
...
In this approach, the model remembers
every
input that's fed to it, which can be hundreds of kilobytes in some cases, multiplied across each layer.
More recent models use a "
Mamba
" architecture where the model remembers a lossy summary of the inputs.
...
The
Nemotron-H-47B
can hold over a million tokens in 32 GB of VRAM ("GPU RAM", roughly) when quantized  to 4-bit weights.
...
In case you skimmed it, that's $42 per
billion tokens
.
A token is about two-thirds of a word.
Books have about 80k words on average.
So this is about 3 cents to read 5 books, or $42 dollars to read 1/10000 of
...
One example is
jgrep
, which allows you to run queries like this:
...
Jev triage
is a TUI for looking at open pull requests, sorted by priority.
It determines priority not by labels, but by
looking at every comment
.
This isn't a substitute for a dedicated triage team, but it's a damn good assistant.
Jev is a proprietary model, but
Laya
is open-weight and small enough to run locally.
...
Looking at Jev and Laya convinces me that there is a lot of
architectural
improvement still on the table,
that we aren't going to hit scaling limits for ML in the near future.
Putting it together
If we combine all this, we see about
2.5 orders of magnitude
decrease in token cost in the last year.
...
Specialized models such as Jev and Laya allow decreasing the cost by another 1-2
orders of magnitude
.
...
What happens next?
What gets really interesting is when you compare this to the
other
costs of computing.
For example, let's look at how expensive tool calls are.
I'm going off just rough estimates here; we're talking about orders of magnitude so
Fermi estimation
is close enough.
GPT-5.6 Luna costs about 30 cents per million tokens (
source
).
...
My MacBook Air draws about 10 W idle and 30 W under heavy use. 
That gives us a table that looks like this:
Tool
Power (W)
Duration (s)
Price (¢)
Orders of magnitude cheaper than Luna turn
grep
10
0.1
0.000007
...
Once models are cheaper than a tool, it becomes attractive to put models
in
tools.
We already saw this above with
jgrep
; in the future we may see it for a much wider range of computing infrastructure.
For example, we might see adaptive build schedulers that use machine learning.
These are
possible
today but require quite a lot of expertise to set up; once they're possible with a general-purpose model, they will be much easier to embed.
Supply-side Jevons Paradox
As models get cheaper to run, companies respond by
building more compute
.
Why? Because they make more money per dollar invested.
This is called the
Jevons Paradox
: the more efficient something is, the more of it exists overall.
In particular, as things get cheaper, people want to use it more.
This is called
induced demand
and often comes up when talking about transport networks.
...
Just because
volume
is getting cheap doesn't mean that
quality
is.
...
If they do,
that
will hurt investors and possibly have ripple effects in the US economy.
...
Cybersecurity is going to get really bad
.
...
This is already happening with services such as
runpod
.
...
Use it.
Don't use it.
Use another similar product.
Now they have a fourth option, which is to tell an LLM to build it.
The
quality
of the LLMs output may be better or worse, but the option is there when it wasn't before.
Companies have to compete on quality, not just on raw ability to do the thing where you couldn't before.
Incumbents in regulated industries will have a massive advantage compared to the free market .
What's really cool about this is it makes it much easier to create
malleable software
that's tailor-made to the exact person using it,
something that would have been unthinkable even 5 years ago for anyone who's not a programmer .
Summary
I don't know what's next.
I do think we should plan for a world where we don't just see cheap
compute
but also cheap
intelligence
.
