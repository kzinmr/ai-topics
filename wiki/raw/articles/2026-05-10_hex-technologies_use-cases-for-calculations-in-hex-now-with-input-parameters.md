...
# Unleashing the Power of Calculations in Hex: New Use Cases and Input Parameters | Hex 

**Source**: [https://hex.tech/blog/use-cases-for-calculations-in-hex-now-with-input-parameters/](https://hex.tech/blog/use-cases-for-calculations-in-hex-now-with-input-parameters/)
...
linkedin
In this article
Quick analysis and modeling
Creating segments for visualization
Simplifying complex filter logic
Bonus: Build interactive data apps
...
Here’s three of the most common use cases for Calculations we’ve heard about from our customers — and a bonus one that’s newly possible with a feature we’re releasing today! See our
full July bundle of product updates
.
Quick analysis and modeling
...
Nikita, a Risk Analyst from Chime, uses calculations for all kinds of math and statistical modeling. She tells us:
“Using Calcs is just faster. I’ve already done the work to get data [from the warehouse] into a table, and with Calculations there’s no need to go into code.”
Calcs run with a real-time output preview, so it feels efficient to prototype even complex formulas. Once you finish, everything automatically compiles to SQL on the backend so your calculation can run on data of any size — even millions, or
billions
of rows.
Creating segments for visualization
...
Liz Kennedy, Principal Marketplace Operations Manager at Upside uses Calculations to find underlying causes for incidents with site operations:
“There’s a series of checks we need to run through when a site isn’t processing correctly. I set up nested IF() statements in a calc, and it makes that really easy.”
...
Calculations can now be “parameterized” with values from Hex’s
Input Parameters
by referencing the name of a parameter with curly brackets:
{{parameter}}
...
See our
July bundle of product updates
.
Share:
twitter
linkedin
