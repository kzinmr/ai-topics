---
title: "In praise of grobi for auto-configuring X11 monitors (2025)"
url: "https://michael.stapelberg.ch/posts/2025-05-10-grobi-x11-monitor-autoconfig/"
fetched_at: 2026-04-30T07:01:06.471073+00:00
source: "michael.stapelberg.ch"
tags: [blog, raw]
---

# In praise of grobi for auto-configuring X11 monitors (2025)

Source: https://michael.stapelberg.ch/posts/2025-05-10-grobi-x11-monitor-autoconfig/
...
Neumann
again and was delighted to discover that
it makes using my fiddly (but wonderful)
Dell 32-inch 8K monitor
(UP3218K)
monitor much more convenient — I get
...
The monitor was connected. My
sleep program
would power on the
monitor (if needed), sleep a little while and then run
xrandr(1)
to (hopefully) configure the monitor correctly.
The monitor was not connected, for example because it was still connected to
my work PC.
In scenario ②, or if the one-shot configuration attempt in scenario ① fails, I
would need to SSH in from a different computer and run
xrandr
manually so that
...
Whenever
grobi
detects that my monitor is connected (it listens for
X11
RandR
output change events), it will run
xrandr(1)
to
configure the monitor resolution and positioning.
To check what
grobi
is seeing/doing, you can use:
...
output change events, which will trigger
grobi
, which will run
xrandr
, which
...
Why not autorandr?
No particular reason. I knew
grobi
.
If nothing else,
grobi
is written in Go, so it’s likely to keep working
smoothly over the years.
Does grobi work on Wayland?
Probably not. There is no mention of Wayland over on the
grobi
repository
.
...
Printf
(
"[monitor power] skipping because -no_monitor flag is set"
)
return
nil
}
...
Printf
(
"unexpected HTTP status code: got %v, want %v"
,
resp.Status,
http.StatusOK)
time.
...
Printf
(
"[monitor power] request succeeded"
)
return
nil
}
...
Sprintf
(
"sudo rtcwake -m no --verbose --utc -t %v && sudo systemctl suspend"
,
wakeup.
Unix
()))
