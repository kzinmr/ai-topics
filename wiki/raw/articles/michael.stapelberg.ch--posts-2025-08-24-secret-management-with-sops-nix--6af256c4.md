...
fetched_at: 2026-05-01T07:01:14.788997+00:00
source: "michael.stapelberg.ch"
tags: [blog, raw]
---

# Secret Management on NixOS with sops-nix (2025)
...
a password somewhere — for example, when I
migrated my existing Linux Network
...
The basic idea behind Secret Management systems is to
encrypt
the secrets at
...
Encrypt the secrets such that other people working on this config can decrypt
them.
...
sops
is a tool to version-control secrets in git, in
...
sops-nix
provides a way to integrate sops
with Nix/NixOS
Using sops with
age(1)
...
age. If you’re curious about agenix,
check out Andreas Gohr’s blog post about
agenix
.
Step 1. Preparation
I ran the following instructions on an
Arch Linux machine on which I installed
...
I don’t want to manage an extra key file, so I’ll use
ssh-to-age
to derive a
...
│   ├── Makefile
…
In the root of the git repository (next to the
batchn
directory), I create
.sops.yaml
like so:
...
path_regex
:
batchn/secrets/[^/]+\.(yaml|json|env|ini)$
key_groups
:
-
age
...
-
*server_batchn
The more systems I manage, the more
keys
and
creation_rules
I will need to
...
in our configured editor by running:
...
Now, we need to reference the encrypted file in NixOS and enable
sops-nix
...
, we tell
sops-nix
to use the SSH host key as
identity, where sops will find our secrets and which secrets
sops-nix
should
...
batchn ~ %
Usage Examples
Now that we have secrets stored in files under
/run/secrets
, how can we use
these secrets?
...
runtime. Because the
ExecStart
directive is interpreted by systemd and not
passed through a shell, we use the
writeShellScript
helper
and then just
cat
...
secrets
.
"fortuneserver/securecookie_hash_key"
=
{
owner
=
...
secrets
.
"fortuneserver/securecookie_block_key"
=
{
owner
=
...
the option might not always be called
EnvironmentFile
. For example, for the
...
In the previous examples, we configured the
owner
of each secret to the user
account under which the service is running. But what if there is no such user
account, because the service use systemd’s
DynamicUser
feature?
We can use systemd’s
LoadCredential
feature! For example, I supply the SMTP
...
=
"/run/credentials/alertmanager.service/smtp_pw"
;
};
...
with an
ExecStartPre
shell script (which is very similar to the techniques
already explained).
Conclusion
