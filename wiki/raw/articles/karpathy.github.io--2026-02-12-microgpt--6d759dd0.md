...
fetched_at: 2026-05-05T07:02:10.219079+00:00
...
BOS
initates a new name, and that another
BOS
...
Every time you do math with
Value
objects (add, multiply, etc.), the result is a new
Value
that remembers its inputs (
_children
...
Here’s a concrete example. Note that
a
...
# 2.0 (dL/db = a = 2)
This is exactly what PyTorch’s
.backward()
gives you:
import
torch
...
on
L
. If you wiggle the inmput
a
, in what direction is
L
changing? Here, the derivative of
L
w.r.t.
a
is 4.0, meaning that if we increase
a
by a tiny amount (say 0.001),
L
would increase by about 4x that (0.004). Similarly,
b.grad = 2.0
means the same nudge to
b
would increase
L
...
return
logits
The function processes one token (of id
token_id
) at a specific position in time (
pos_id
), and some context
from the previous iterations summarized by the activations in
keys
and
values
...
t
gets to “look” at tokens in the past
0..t-1
...
Both the attention and MLP blocks add their output back to their input (
x = [a + b for ...]
). This lets gradients flow directly through the network and makes deeper models trainable.
Output.
The final hidden state is projected to vocabulary size by
lm_head
...
Tokenization.
Each training step picks one document and wraps it with
BOS
on both sides: the name “emma” becomes
[BOS, e, m, m, a, BOS]
