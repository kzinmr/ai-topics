...
fetched_at: 2026-05-05T07:01:45.254083+00:00
...
Source: https://maskray.me/blog/2025-08-31-benchmarking-compression-programs

tl;dr
https://gist.github.com/MaskRay/74cdaa83c1f44ee105fcebcdff0ba9a7
...
lz4, snappy, Oodle Selkie
.
Medium ratio, medium speed:
zlib, zstd,
brotli
, Oodle
Kraken
...
LZMA, bzip2, bzip3, bsc, zpaq,
kanzi
, Oodle Leviathan
.
Low ratio
Codecs in this category prioritize speed
...
categories. For example, a high-level
zstd
compression can
achieve a ratio comparable to
xz
(a high-compression codec) by
using more RAM and CPU. While
zstd
's compression speed or ratio
...
lz4
: Focuses on speed over compression ratio. Memory usage
is extremely low. It seems Pareto superior to Google's
Snappy
.
zstd
: Gained significant traction and obsoleted many
...
HTTP header. Decompression memory
usage is very low.
brotli
: Uses a combination of LZ77, 2nd order context
...
overshadowed by
zstd
. Compared with DEFLATE, it employs a
...
is supported by the
Accept-Encoding: br
HTTP header. Decompression memory usage
is quite low.
bzip3
: Combines BWT, RLE, and LZP and uses arithmetic
encoder. Memory usage is large.
xz
: LZMA2 with a few filters. The filters must be enabled
explicitly.
...
.
zpaq
: Functions as a command-line archiver supporting
multiple files. It combines context mixing with arithmetic encoder but
operates very slowly.
kanzi
: There are a wide variety of transforms and entropy
encoders, unusual for a compresion program. For the compression speed of
enwik8, it's Pareto superior to
xz
, but decompression is
...
trouble getting it to compile due to a
cstdio
header
issue.
Many modern compressors are parallel by default. I have to disable
this behavior by using options like
-T1
. Still,
zstd
uses a worker thread for I/O overlap, but I don't bother
with
--single-thread
.
...
hsl
function in CSS).
The single file Ruby program is available at
https://gist.github.com/MaskRay/74cdaa83c1f44ee105fcebcdff0ba9a7
Limitation
...
process, resulting in faster decompression. Some programs, like
zpaq
and
kanzi
, use different algorithms that can
result in significantly different decompression speeds.
xz -9
