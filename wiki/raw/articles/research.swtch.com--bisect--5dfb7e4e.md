...
fetched_at: 2026-05-05T07:01:00.991136+00:00
...
Source: https://research.swtch.com/bisect
...
Here is an example I wrote long ago for Go’s
sort.Search
documentation:
func GuessingGame() {
    var s string
...
“
so basic that there is no need to write them down
.”
...
% git bisect run sh -c '
    ...
    go test -count=5
'
It takes some care to write a correct
git bisect
invocation,
but once you get it right, you can walk away while
git bisect
works its magic.
In this case, the script we pass to
git bisect run
cleans out any stale files
and then builds the Go toolchain (
./make.bash
).
If that step fails, it exits with code 125,
a special inconclusive answer for
git bisect
:
...
If
go list
fails, which happens if the bisect uses a version of Go that’s too old,
the script exits successfully, indicating that the bug is not present.
Otherwise the script runs
go test
and exits with the
status from that command. The
-count=5
is there because
...
The culprit commit that
git bisect
identified is a significant change to the timer
implementation.
...
That is, we can run binary search on a different axis: over the
program’s code
, not its version history.
We’ve implemented this search in a new tool unimaginatively named
bisect
.
When applied to library function behavior like the timer change,
bisect
can search over all stack traces leading to the new code,
...
Here the
bisect
tool is reporting that disabling
asynctimerchan=1
...
In contrast, this
bisect
tool is showing the stack at the moment
just
before
things go wrong:
...
This tells us that the seventh function,
sin
, is one function that must
be optimized to provoke the failure.
More precisely, with
sin
optimized, we know that
...
over the other remaining six list entries, always adding
sin
as well:
This time, optimizing the first two (plus the hard-wired
sin
) fails,
...
Then we have just one suspect location left:
add
.
A binary search over that one-entry list (plus the two hard-wired
cos
and
sin
)
shows that
add
can be left off the list without losing the failure.
...
makes the test failure disappear: optimizing
cos
or
sin
by itself is not enough.
However, the set may still not be globally minimal:
perhaps optimizing only
tan
would cause a different failure (or not).
...
But now
cos
is never optimized (it just got discarded),
...
For simplicity, let’s assume we have a global function
buggy
...
BisectReduce
takes a single input list
targets
for which
buggy(targets)
is true and returns a locally minimal subset
x
for which
buggy(x)
remains true. It invokes a more generalized
helper
bisect
, which takes an additional argument: a
forced
...
right
.
Applied to the function optimization example,
BisectReduce
would end up at a
call to
...
The version of
BisectReduce
we have been considering is the shortest
...
Those flags are
still present today (in debug builds)
,
and you can find uses of them in
Java bug reports dating back at least to early 2000
.
...
to accept a list of integer ranges, like
-XX:CIAllow=1-5,7-10,12,15
.
...
First we have to adjust the definition of
buggy
:
...
In this version,
bisect
does not guarantee to cut the search space in half
...
For example, a hypothetical Java compiler could use
-XX:CIAllowHash=000,10,111
.
...
Keith introduced an
environment variable GOSSAHASH
...
He
reused the same technique
but at finer granularity:
the bit pattern now controlled which functions that rewrite rule
could be used in.
David also wrote the
initial version of a tool,
gossahash
,
for taking on the job of binary search.
Although
gossahash
only looked for a single failure, but it was remarkably helpful.
It served for many years and eventually became
bisect
.
...
For example, this bisection finds that
b.go:7
is the culprit line:
...
Like those languages, Go recently changed to
iteration-scoped loop variables
,
...
We arranged for that framework to provide
bisect
support for all
GODEBUG settings automatically.
For Go 1.23, we rewrote the
time.Timer
...
bisect
made the process painless and maybe even fun.
...
to run every trial
N
times and bail out entirely if they disagree.
We set the default to
-count=2
so that
bisect
always does basic
...
running
go test -count=5
increases the chance of failure by running the test five times.
...
Now bisect only sees
./flakytest
passing five times in a row as a successful run.
...
A more principled approach inside
bisect
would be better.
...
decision prints only the hash bits, since that’s all
bisect
needs
to guide the search and narrow down the relevant stacks.
Once
bisect
has identified a minimal set of relevant stacks,
...
and
bisect
displays those stack traces in its report.
Try Bisect
The
bisect
tool
...
and
-godebug
flags are conveniences.
The general form of the command is
% bisect [KEY=value...] cmd [args...]
where the leading
KEY=value
arguments set environment variables
before invoking the command with the remaining arguments.
Bisect
expects to find the literal string
PATTERN
somewhere
...
You can use
bisect
to debug problems in your own compilers or libraries
by having them accept a hash pattern either in the environment or on
the command line and then print specially formatted lines for
bisect
...
(there is a
pending proposal
to add it to the Go standard library),
but the package is only a
