...
fetched_at: 2026-04-28T07:01:51.622422+00:00
...
Source: https://michael.stapelberg.ch/posts/2025-11-29-self-hosting-photos-with-immich/

Table of contents
For every cloud service I use, I want to have a local copy of my data for backup
purposes and independence. Unfortunately, the
gphotos-sync
tool
stopped
working in March
...
Immich
, a self-hostable photo manager.
Here is the end result: a few (live) photos from
NixCon
2025
:
Step 1. Hardware
I am running Immich on my
Ryzen 7 Mini PC (ASRock DeskMini
X600)
, which
...
NixOS
on that VM as described in this blog post:
...
enabled, that means I can now open
...
Unfortunately, these files are not considered by
immich-cli
.
...
I ran
immich-go
as follows and it worked beautifully:
% immich-go \
  upload \
  from-google-photos \
...
notifications are not required for background upload to work, as
an Immich
...
→
Notifications
and un-tick the permission checkbox:
Step 4. Backup
...
on NixOS. The
backups
subdirectory contains SQL dumps, whereas the 3 directories
upload
,
library
and
profile
contain all user-uploaded data.
Hence, I have set up a systemd timer that runs
rsync
to copy
/var/lib/immich
onto my PC, which is enrolled in a
3-2-1 backup
scheme
.
What’s missing?
Immich (currently?) does not contain photo editing features, so to rotate or
crop an image, I download the image and use
GIMP
.
