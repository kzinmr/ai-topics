...
fetched_at: 2026-07-05T07:00:45.286725+00:00
...
While I’m not
entirely sure
that this is how it works, there are some
...
You can
ask
the model to produce valid JSON matching a schema and then
...
Across repeated trials I saw a whole zoo of invented trailing keys:
type
,
id
,
...
The most annoying part is that the actual
oldText
and
newText
payloads were
...
that ecology is not documented.  While there is a
text editor
tool
...
data.
For a start, Claude Code checks the model’s visible text for leaked
<invoke
...
It has explicit Unicode escape repair which fixes broken
\uXXXX
sequences and
...
were trained on the officially documented text editor tool), the newer
old_string
from the schema,
new_str
/
...
file_path
, and some more.
It also silently filters out unexpected keys and it does not use
strict
mode
either.  The issue with
strict
...
a bit of this also happens in Anthropic’s models, at least in
strict
mode
I would imagine.
...
closing a several-hundred-token escaped
newText
...
harnesses are.  Obviously one could turn on
strict
sampling in
