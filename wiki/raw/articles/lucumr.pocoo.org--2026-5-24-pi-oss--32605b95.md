...
fetched_at: 2026-05-25T07:01:33.216005+00:00
...
Pi
is now part of Earendil, but in the important sense it is
still
Mario’s
project.  He has been living with its
...
I might hand to my clanker and say: “understand this, reproduce it, inspect
...
code references look plausible.  We use a custom slash command called
/is
,
...
analyze
is
sue) is a prompt for analyzing GitHub issues: it labels and assigns
...
code.  Then an extension adds a
prompt-url-widget
which watches the prompt
before the agent starts, recognizes the GitHub issue or PR URL that
/is
(or
the PR equivalent) put into the prompt, fetches the title and author with
gh
,
...
running
/is
against a different issue, and the UI keeps the investigations
...
sequentially.  To finish off everything,
/wr
(
wr
ap it up) is the matching
...
only the files changed in that session, adds the appropriate
closes #...
when
there is exactly one issue, and pushes from
main
.
