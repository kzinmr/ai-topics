...
fetched_at: 2026-05-05T07:01:35.896609+00:00
source: "LLVM Blog"
tags: [blog, raw]
---
...
Source: https://blog.llvm.org/posts/2025-gsoc-clangir-upstreaming/
...
Starting from April 2024, the upstreaming process started to migrate the code from the
incubator
to the
main
LLVM repository.
...
and
ext_vector_type
attributes) with operators and built-ins functions.
ComplexType with operators and all
-complex-range
flag values to allow selecting different arithmatic calculation algorithms, for example, the SMITH, R. L. Algorithm 116: Complex division. Commun. ACM 5, 8 (1962).
...
and
__imag__
unary operators as MLIR operations not only when applying them on Complex but also on scalar values, for example.
...
Also, all
-complex-range
flag values to allow selecting different arithmatic calculation algorithms are fully implemented, for example, to use the
...
With that representation that abstracts the actual calculations of the complex multiplications at that stage, it’s easy to write a folder that, in the case of the element type of complex, is integer and one of the operands is
{0, 0}
, then we can fold the ComplexMulOp to just complex with 0 as real and imaginary values 🤓.
Also implemented support for using ComplexType with all C/C++ expressions except expressions related to OpenMP or Coroutine because they are not fully implemented, and implemented floating-point operators for built-in functions, for example,
acos
,
asign
,
atan
…etc, which are required to implement future built-in functions for ComplexType.
...
Also, I implemented support for rethrow and throw with sub-expression, for example,
throw _builtin_complex(1.0f, 2.0f)
.
...
implemented in the classic Clang CodeGen (
PR in Clang
).
Some ComplexType built-in functions should be implemented later once the call convention is upstreamed, for example,
__builtin_cabs
,
__builtin_ccos
, …etc.
...
I was impressed by my knowledge near the end of Google Summer of Code when I found a crash in the original Clang codegen, and in almost one hour, I was able to find the exact reason for the crash and create a PR to fix it (
#160583
).
...
Contributions & PRs
PR’s in the main LLVM repo for upstreaming stuff or implementing features that will be backported to the ClangIR (
191 PR
).
PR’s in the ClangIR repo for implementing features or backporting work from the upstream (
75 PR
).
The fix to the classic clang codegen (
1 PR
).
