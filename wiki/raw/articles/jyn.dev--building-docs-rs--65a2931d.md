...
Source: https://jyn.dev/building-docs-rs/

What is docs.rs?
docs.rs
is a site dedicated to hosting documentation for Rust projects.
...
rustdoc
is tied so closely to the compiler,
this requires building every package from source using
cargo doc
.
As a result, I've heard several members of the docs.rs team describe it as
'Remote Code Execution as a Service' (since
build.rs
files can
...
docs.rs has to
store
all this documentation somewhere. In fact, it's in
an AWS storage bucket and gets uploaded after every build.
How did I get involved?
My main rust project is
rcc
, a C compiler written in Rust.
I use
Cranelift
to generate the assembly
code; Cranelift is a code generator like
LLVM
.
...
Firefox got convinced that I always wanted to visit
0.36
- see also the
rustdoc issue
...
What happened?
At that point, docs.rs was using
Vagrant
to run
...
switched to a different sandbox
,
...
writing a
Dockerfile
and Pietro was so impressed that he started
using it in production
:D
After a month and what has been called
Unprecedented (but greatly appreciated) levels of yak shaving
I finally
got the original change merged
:)
What next?
Pietro and
QuietMisdreavus
...
to
docs.rs
:) (Pietro still handles the deploys)
