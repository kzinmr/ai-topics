---
title: "FORTIFY_SOURCE - Idea of the day"
url: "https://idea.popcount.org/2013-08-15-fortify_source"
fetched_at: 2026-05-05T07:01:13.080405+00:00
...
Source: https://idea.popcount.org/2013-08-15-fortify_source
...
-fstack-protector
option GCC adds protection code to your functions that use
alloca
or
...
x10
(
%ebp)  ; save eax in the stack frame
mov    -0xc(%
ebp
),
%eax   ; load canary
...
When the stack is overwritten and
__stack_chk_fail
branch is taken
...
Compiled without the code fortified, with
-U_FORTIFY_SOURCE
option:
08048450
<
...
0x8
(
%esp)   ; save value 0x100 as parameter
mov    0x8(%
ebp
),
%eax     ; load parameter `s` to eax
mov    %
eax
,
0x4
(
%esp)     ; save parameter for strcpy
lea    -0x108(%
ebp
),
%eax  ; count `buf` in eax
mov    %
eax
,(
%esp
...
FORTIFY_SOURCE can also help with
strncpy
when GCC knows the
