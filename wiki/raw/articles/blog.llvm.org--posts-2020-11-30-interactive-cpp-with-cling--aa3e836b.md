...
Source: https://blog.llvm.org/posts/2020-11-30-interactive-cpp-with-cling/
...
an interactive C++ interpreter, built on top of the Clang and
LLVM
...
ROOT
. ROOT is a set of interconnected components that assist
...
In short:
The tool controls the input infrastructure by interactive prompt or by an
interface allowing the incremental processing of input (➀).
It sends the input to the underlying clang library for compilation (➁).
Clang compiles the input, possibly wrapped into a function, into an AST (➂).
When necessary the AST is further transformed in order to attach specific
behavior (➃).
...
v
can be referenced by subsequent inputs. Wrapper #2 contains a statement and
...
An integral part of interactivity is printing expression values. Typing
printf
...
corresponding wrapper AST does not contain a
NullStmt
(modelling extra
semicolons).
[cling]
...
providing a ‘managed’ storage. The managed storage (
cling::Value
) is also used
...
std::vector<int> v
input_line_7:2:19: error: redefinition of
'v'
...
(
ACM conference on Compiler Construction
).
We enable it by calling
...
Transaction
. A transaction represents the delta of the changes of internal
...
=
nullptr; *p
input_line_3:2:21: warning: null passed to a callee that requires a non-null argument
[
-Wnonnull
]
int *p
...
4
In the example, we include a header file with the
.L
meta command;
“uninclude” it with
.U
and “reinclude” it with
.L
to re-read the modified
