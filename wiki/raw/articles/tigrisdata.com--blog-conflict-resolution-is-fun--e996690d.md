...
url: "https://www.tigrisdata.com/blog/conflict-resolution-is-fun/"
fetched_at: 2026-09-07T10:01:45.438073+00:00
source: "xeiaso.net"
tags: [blog, raw]
---
...
Source: https://www.tigrisdata.com/blog/conflict-resolution-is-fun/
...
As a result, we
really
want to have our own business logic define which side
...
effect this also means that the data
does not
move between regions like other
...
│ a read-only copy              │
└───────────────────────────────┘
The main tradeoff with a single-region bucket is that you trade
strict
consistency
(because there is only one possible writer) with
higher latency
...
of the
most complicated things in computing
.
...
function:
FIG 02
compare() decides which version of an object wins
compare(previous, new, force)
│
├──
is prev or next unset, or a brand-new key in FDB?
│
yes: different objects
──────────▶
drop, log the conflict  ✗
│
├──
is prev older than the new data?
│
...
│
└──
tie, or new is older: is this a forced change?
...
after
the delete was committed locally.
...
FIG 04
a clock-skew race resurrects a deleted object
...
stamped …000200
│                       │                       │
(the clock ran a hair fast)
│                       │                       │
│                       │ replicate @ …000200   │
│                       ├──────────────────────▶│
│                       │                       │
Singapore stores the
│                       │                       │
live copy @ …000200
│                       │                       │
│ DELETE thing.txt      │                       │
├──────────────────────▶│                       │
│                       │                       │
the clock still reads …000100,
│                       │                       │
so the tombstone is stamped
│                       │                       │
BEHIND the row it deletes
│ 204 No Content        │                       │
│◀─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┤                       │
│                       │                       │
│                       │ tombstone @ …000100   │
│                       ├──────────────────────▶│
│                       │                       │
…000100 is not newer than
│                       │                       │
…000200: the tombstone is
│                       │                       │
dropped, the live copy survives
│                       │                       │
│ GET thing.txt         │                       │
├───────────────────────┼──────────────────────▶│
│
200 OK, the deleted object is back
│
│◀─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┼ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ┤
│                       │                       │
│ PUT If-None-Match: *  │                       │
...
FIG 05
a multi-region write with one follower down
...
│               ├──────────────▶│               │               │
│               │               │               │               │
upload to the leader's block
│               │               │               │               │
store, commit the metadata:
│               │               │               │               │
ONE transaction writes the row
│               │               │               │               │
and enqueues replication
│               │
committed
│               │               │
│               │◀─ ─ ─ ─ ─ ─ ─ ┤               │               │
...
·               ·               ·               ·               ·
│               │               │               │               │
seconds later, the queue delivers
│               │               │               │               │
the same change all over again
│               │               │ queued copy   │               │
│               │               ├──────────────▶│               │
│               │               │               │               │
...
│               │               ├───────────────┼──────────────▶│
│               │               │               │               │
nothing here yet, so it applies:
│               │               │               │               │
...
with life:
FIG 06
the failed write that succeeds anyway
...
│                 ├────────────────▶│                   │
│                 │                 │                   │
the row is written and durable.
│                 │                 │                   │
...
│                 │                 │                   │
│
5xx, your write failed
│                   │
│◀─ ─ ─ ─ ─ ─ ─ ─ ┤                 │                   │
│                 │                 │                   │
no follower succeeded
·                 ·                 ·                   ·
│                 │                 │                   │
but the queued copy was already
│                 │                 │                   │
durable, committed in the same
│                 │                 │                   │
...
│                 │                 │                   │
│                 │                 │                   │
the write you were told failed is
│                 │                 │                   │
now readable in every region
...
cluster!
FIG 03
sharding FoundationDB clusters into replicasets
S3 request
│
▼
┌─────────┐
── which replicaset holds this tenant? ──▶
┌────────────────┐
...
Need your own replicaset on Tigris?
If your workload is big enough, you could end up with a dedicated FoundationDB cluster all to yourself. Give us a shout and let's see if your data qualifies.
