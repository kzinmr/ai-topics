...
Source: https://blog.llvm.org/posts/2025-03-11-flang-new/
...
until recently the Flang binary was not
flang
(like
clang
) but instead
flang-new
.
LLVM 20 ends the era of
flang-new
. The community has decided that Flang is
worthy of a new name.
The “new” name? You guessed it,
flang
.
...
HPC compilers
. Then
there are open source options like
GFortran
, which
...
1990
Intel bundles PGI compilers with its iPSC/860 supercomputer.
1996
PGI
works with
...
STMicroElectronics.
August 2011
Away from PGI, Bill Wendling
starts
an LLVM based Fortran compiler called “Flang” (later known as
“Fort”
).
...
November 2015
NVIDIA joins the US Department of Energy
...
May 2017
The first release of Classic Flang as a separate
...
Project.
December 2017
The original “Flang” is renamed to
“Fort”
...
August 2018
Eric Schweitz (NVIDIA) begins work on what would become
...
code (later
fir-dev
work addressed this). Despite that, it moved into
flang/
...
of the
flang.sh
driver wrapper script (more on this later).
December 2019
The
plan
...
April 2020
Upstreaming of F18 into the LLVM monorepo is
completed
.
...
Flang binary is named
flang
, just like the folder F18 now occupies in the LLVM Project.
Note:
Some LLVM changes referenced below will appear to have skipped an LLVM release.
...
April 2020
The first attempt at adding a new compiler driver for Flang is
posted
for review. It used the name
flang-tmp
. This change was later abandoned in favour of a different approach.
September 2020
Flang’s new compiler driver is
added
as an experimental option. This is the first appearance of the
flang-new
binary,
instead of
flang-tmp
...
October 2020
Flang is included in an LLVM release for the first time in
LLVM 11.0.0. There is an
f18
binary and the previously mentioned script
flang.sh
.
August 2021
flang-new
is no longer experimental and
replaces
the previous Flang compiler driver binary
f18
.
October 2021
LLVM 13.0.0 is the first release to include a
flang-new
binary
(alongside
f18
...
of the
fir-dev
branch in the Classic Flang project. Upstreaming of
fir-dev
...
option is used. This change used work
originally done on the
fir-dev
branch.
May 2022
Kiran Chandramohan
...
The
flang.sh
compiler driver script becomes
flang-to-external-fc
. It
allows the user to use
flang-new
to parse Fortran source code, then write it back
...
option.
July 2022
Valentin Clément (NVIDIA)
announces
that upstreaming of
fir-dev
to the LLVM Project is complete.
September 2022
LLVM 15.0.0 releases, including Flang’s experimental code
generation option.
September 2023
LLVM 17.0.0 releases, with Flang’s code generation enabled
by default.
...
from
fir-dev
. As opposed to “Classic Flang” that combines a PGI based frontend and
...
.
September 2024
LLVM 19.1.0 releases. The first release of
flang-new
as a standalone compiler.
October 2024
The community deems that Flang has met the criteria to not be
“new” and the name is changed. Goodbye
flang-new
, hello
flang
!
...
March 2025
LLVM 20.1.0 releases. The first time the
flang
binary has been
...
proposal
Brad Richardson decided to use the
LLVM proposal process
.
...
Confusion about the
flang
name was a key motivation for Brad Richardson too:
...
concern was valid, because right now [November 2024] fpm [
...
the project. It became its own public branch
fir-dev
, which was later contributed
to the LLVM Project.
...
func
@_QPexample
(%arg0: !fir.box<!fir.array<
?x
f32
>> {fir.bindc_name =
"a"
}, %arg1: !fir.box<!fir.array<
?x
f32
>> {fir.bindc_name =
"b"
...
%1:2 = hlfir.declare %arg0 dummy_scope %0 {uniq_name =
"_QFexampleEa"
} : (!fir.box<!fir.array<
?x
f32
>>, !fir.dscope) -> (!fir.box<!fir.array<
?x
f32
>>, !fir.box<!fir.array<
?x
f32
>>)
%2:2 = hlfir.declare %arg1 dummy_scope %0 {uniq_name =
"_QFexampleEb"
} : (!fir.box<!fir.array<
?x
f32
...
f32
>>)
hlfir.assign %2#0 to %1#0 : !fir.box<!fir.array<
?x
f32
>>, !fir.box<!fir.array<
?x
...
arguments of a subroutine
)
are declared with
hlfir.declare
but their type is specified with
fir.array
.
...
:
i32
%7 = fir.convert %0 : (!fir.ref<!fir.box<!fir.array<
?x
f32
>>>) -> !fir.ref<!fir.box<none>>
%8 = fir.convert %5 : (!fir.box<!fir.array<
?x
f32
>>) -> !fir.box<none>
%9 = fir.convert %6 : (!fir.ref<!fir.char<
1
,
17
>>) -> !fir.ref<
...
%10 = fir.call
@_FortranAAssign
(%7, %8, %9, %c3_i32) : (!fir.ref<!fir.box<none>>, !fir.box<none>, !fir.ref<
i8
>,
i32
) -> none
...
@_FortranAAssign
(ptr %5, ptr %3, ptr
@_QQclX2F6170702F6578616D706C652E66393000
,
i32
3
)
...
OpenMP to Everyone
Note:
Most of the points made in this section also apply to
OpenACC
support in Flang. In the interest of brevity, I
...
OpenMP Basics
OpenMP
is a standardised API for adding
...
Fortran arrays are
one-based
by default. So the first element is at index 1. This example reads the previous element as well, so it starts
I
at 2.
!$OMP PARALLEL DO
is a directive in the form of a Fortran comment (Fortran
comments start with
!
).
PARALLEL DO
starts a parallel “region” that
includes the code from
DO
to
ENDDO
.
This tells the compiler that the work in the
DO
loop should be shared amongst
...
For C++, the directive is in the form of a
#pragma
and attached
to the
for
...
(
i32
noundef %n, ptr noundef %a, ptr noundef %b) <...> {
entry:
<...>
call void (<...>)
@__kmpc_fork_call
...
The body of
simple
no longer does all the work. Instead it uses
__kmpc_fork_call
to tell the OpenMP
runtime library
to run another function,
simple (.omp_outlined)
to do the work.
...
OMP_NUM_THREADS
.
The LLVM IR representation of
simple (.omp_outlined)
includes labels like
omp.loop.exit.i
, but these are not specific to OpenMP. They are just normal LLVM IR
labels whose name includes
omp
.
...
This proposal led to the creation of the
LLVMFrontendOpenMP
library in
llvm
. By using the same class
OpenMPIRBuilder
, there is no need to repeat work in
...
Early in 2020, Kiran Chandramohan (Arm)
proposed
...
@_QPsimple
(<...> {
%1:2 = hlfir.declare %arg0 <...> {uniq_name =
"_QFsimpleEn"
} : <...>
%3:2 = hlfir.declare %2 <...> {uniq_name =
"_QFsimpleEi"
} : <...>
%10:2 = hlfir.declare %arg1(%9) <...> {uniq_name =
"_QFsimpleEa"
} : <...>
%17:2 = hlfir.declare %arg2(%16) <...> {uniq_name =
"_QFsimpleEb"
} : <...>
%22:2 = fir.do_loop <...> {
<...>
hlfir.assign %34 to %37 :
f32
, !fir.ref<
f32
>
}
fir.store %22#1 to %3#1 : !fir.ref<
i32
>
return
}
...
.
omp
is an MLIR dialect that describes
OpenMP
.
This translation of the
PARALLEL DO
directive is much more literal than
the LLVM IR produced by Clang for
parallel for
.
As the
omp
dialect is specifically made for OpenMP, it can represent
...
uses the same
OpenMPIRBuilder
class that Clang does. From the
MLIR shown previously,
OpenMPIRBuilder
...
Work on OpenMP support for ClangIR has already
started
,
using the
omp
dialect that was originally added for Flang.
Unfortunately at time of writing the
parallel
directive is not supported by
ClangIR. However, if you look at the CIR produced when OpenMP is disabled, you can
see the
cir.for
element that the OpenMP dialect might replace:
module <...> attributes {<...>} {
cir.
func
@_Z6simpleiPfS_
...
Note:
This section paraphrases material from
“Flang Drivers”
.
...
how to handle a set of options. For instance, when you use the option
-march=armv8a+memtag
,
something in Flang knows that you want to compile for Armv8.0-a with the Memory
Tagging Extension enabled.
-march=
is an example of a “compiler driver” option. These options are what users
...
In Flang’s case the “compiler driver” is
flang
and the “frontend driver” is
flang -fc1
(they are two separate tools, contained in the same binary).
...
You can see the differences if you add
-###
...
-march=
has been converted into many
arguments to
flang -fc1
. This means that if compiler developers decided to
change how
-march=
was converted, existing
flang
...
library (
clangDriver
) to implement its own compiler driver.
...
SciPy
is a foundational Python package. It provides easy
...
we switched to using Flang, and that
process
was a minor miracle.
