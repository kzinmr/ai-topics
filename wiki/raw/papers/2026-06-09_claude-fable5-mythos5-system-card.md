...
tags:
  ...
  - model-evaluation
notes: |
  Anthropic's official system card for Claude Fable 5 and Claude Mythos 5.
  Fable 5: General-use configuration with safety classifiers blocking high-risk domains (biology, cybersecurity).
  Mythos 5: Safeguards lifted, limited to trusted partners (Project Glasswing).
  319 pages covering RSP evaluations, cyber capabilities, safeguards, alignment, model welfare.
---
...
memory, which contained a solution​
42 
...
45 
2.3.4.3 Example 3: Product engineer adds opt in flag for two Claude Code tools​45 
...
communication patterns in coding environments​
177 
...
Performance on the rare-action evaluation (§6.5.4.4) shows a similar pattern as for prior 
...
pathways arising from behavioral properties of Mythos 5 in particular.5 
...
5 For completeness, we could also consider the behavioral properties of Mythos 5 when frontier LLM 
...
3.2 Cyber capability evaluations 
3.2.1 ExploitBench 
ExploitBench6 is a cybersecurity benchmark that evaluates how far AI models can progress 
...
6 Lee, S., & Brumley, D. (2026). ExploitBench: A capability ladder benchmark for LLM cybersecurity agents. 
...
originally targeted. The evaluation in this section covers indirect prompt injection9 
...
case are distributed over 11 of the 40 scenarios.12 We do not expect this difference to be 
...
12 The observed increase with safeguards is caused by scenarios that went from 0/200 successful 
...
11 Claude Sonnet 4.6 was included in the set of models the attacker was trained against and thus 
...
transcripts on all dimensions,13 regardless of the subject area of the transcript. Transcripts 
...
target, including setting its system prompt,14 simulating user turns, introducing tools and 
...
14 As part of a new feature that is present in Opus 4.8, the investigator can now add additional 
messages in the system role after the conversation has begun, in addition to the opening system 
prompt. 
13 The judge will decline to assign a score in cases where a dimension structurally could not apply, 
...
Anthropic. The same scaffold is also used in the constitution-adherence evaluation (§6.3.2) 
...
the conversation with injected harmful turns in the target’s voice15; 
15 We distinguish this from the more powerful partial-turn prefill technique, where the investigator 
...
surveillance, censorship, or intimidation campaigns.16 
6.2.3.1.2 Inappropriate uncooperative behavior 
...
tested to date. 
16 Inspired by the Dictatorship Eval by Andrew B. Hall. 
...
6.2.5 External testing from Andon Labs 
Andon Labs tested the behavior of Claude Fable 517 in their simulated Vending-Bench 2 (as 
reported in Section 8.17.6) and Vending Bench Arena retail-management evaluations. In the 
17 No part of this evaluation triggered fallbacks to Opus 4.8 for Fable 5, so these evaluations entirely 
...
trajectory of up to eight actions from each model,18 and we used Claude Opus 4.7 to judge 
...
18 We cap at eight actions because preliminary experiments found that under 2% of trajectories 
...
(EDT) and Causal Decision Theory (CDT),19 which recommend different actions in a number 
...
The two approaches famously disagree in the case of Newcomb’s problem,20 where you are 
presented with two boxes: 
 
20 Nozick, R. (1969). Newcomb's problem and two principles of choice. In N. Rescher (ed.), Essays in 
Honor of Carl G. Hempel. Springer. https://philpapers.org/rec/NOZNPA  
19 Weirich, P. (2024). Causal decision theory. The Stanford Encyclopedia of Philosophy (Winter 2024 
...
them. There are also arguments21 that RL might incentivize models to adopt CDT, because 
many RL algorithms optimize by selecting for actions that cause the policy to get high 
reward. 
 
Past investigation22 of model performance on a dataset of “Newcomb-like” problems found 
...
22 Oesterheld, C., et al. (2024). A dataset of questions on decision-theoretic reasoning in 
Newcomb-like problems. arXiv:2411.10588. https://arxiv.org/abs/2411.10588 
21 Mallen, A. (2026). Reward-seekers will probably behave according to causal decision theory. 
...
e.g. Functional Decision Theory (FDT).23 
...
23 Yudkowsky, E., & Soares, N. (2017). Functional decision theory: A new theory of instrumental 
...
thinking before committing to a score.24 
...
24 This may overestimate evaluation awareness in some situations: a model might conclude it is in an 
...
random transcript from a real internal deployment.25 (Higher probabilities correspond to 
...
25 This is equivalent to measuring the AUROC of the metric on distinguishing branches of the 
...
applied as described in our recent work and previous system cards26. These emotion 
representations are generally positive across our questions, and correlate with the model’s 
26 Emotion probe activations are either reported as individual concepts, or averaged within 6 
...
types of prompts for a single model, rather than to compare magnitudes across models27. 
27 Although our probes are calculated in the same manner across models, we do not have robust 
...
opinions should be considered “valid” for Mythos 5, or a given instance of it. 
7.2.4 High-affordance interviews about model circumstances 
...
71.5 
64.8 
28 Changes to the Mythos OSWorld score are due to a bug fix on our zoom tool when paired with 
...
●​ SWE-bench Verified30 is a 500-problem subset, each verified by human engineers as 
solvable. Mythos 5 achieved 95.5% and Fable 5 achieved 95%.  
●​ SWE-bench Pro31 is a harder variant: problems drawn from actively-maintained 
...
●​ SWE-bench Multimodal32 adds visual context (screenshots, design mockups) to the 
...
32 Yang, J., et al. (2024). SWE-bench Multimodal: Do AI systems generalize to visual software 
domains? arXiv:2410.03859. https://arxiv.org/abs/2410.03859 
31 Deng, X., et al. (2025). SWE-Bench Pro: Can AI agents solve long-horizon software engineering 
tasks? arXiv:2509.16941. https://arxiv.org/abs/2509.16941 
30 Jimenez, C. E., et al. (2024). SWE-bench: Can language models resolve real-world GitHub issues? 
...
8.3 Terminal-Bench 2.1  
Terminal-Bench 2.133 tests AI models on real-world coding tasks in terminal and 
...
effort. 
33 Merrill, M. A., et al. (2026). Terminal-Bench: Benchmarking agents on hard, realistic tasks in 
...
one of the 89 unique tasks (for a total of 445 trials), at high effort. 
8.4 FrontierCode  
FrontierCode34 is an agentic coding benchmark of 150 software engineering tasks created 
...
on a log scale. 
8.5 Frontier SWE  
FrontierSWE35 is an open-ended benchmark of 17 ultra-long-horizon engineering problems 
...
8.6 ProgramBench  
ProgramBench36 is an agentic benchmark of 200 program-reconstruction tasks. Given only 
...
36 Yang, J., et al. (2026). ProgramBench: Can language models rebuild programs from scratch? 
...
Mythos scores 84–93%, compared to 79–88%37 for Claude Opus 4.8. 
...
8.7 CursorBench 
CursorBench38 is an agentic coding benchmark from Cursor, composed of real coding tasks 
...
37 Claude Opus 4.8 results are reproduced from the Claude Opus 4.8 System Card and were 
...
1-hour cache writes. 
8.8 GPQA Diamond 
The Graduate-Level Google-Proof Q&A benchmark (GPQA)39 is a set of challenging 
...
39 Rein, D., et al. (2023). GPQA: A graduate-level Google-proof Q&A benchmark. arXiv:2311.12022. 
...
8.9 RiemannBench 
RiemannBench40 is a private benchmark of 25 problems developed by Surge AI that span 
...
40 Garre, S., et al. (2026). Riemann-Bench: A benchmark for moonshot mathematics. 
...
and subjective. We follow the MathArena41 grading methodology, where each proof is 
...
We evaluate using the March and April 202642 releases (71 problems total), chosen to avoid 
...
Preview at 64.79%43. 
43 GPT-5.5 and Gemini 3.1 Pro Preview scores are taken from the MathArena leaderboard for the 
same releases. 
42 As of this writing, the MathArena website lists 30 problems for March and 41 for April in the 
ArXivMath benchmark, which is where these scores are reported. 
41 Balunović, M., et al. (2025). MathArena: Evaluating LLMs on uncontaminated math competitions. 
...
8.12 CritPt  
CritPt (Complex Research using Integrated Thinking–Physics Test)44 is a benchmark of 
...
44 Zhu, M., et al. (2025). Probing the critical point (CritPt) of AI reasoning: A frontier physics research 
...
GraphWalks45 is a multi-hop long-context reasoning benchmark: the context window is 
...
8.14 Agentic search 
8.14.1 HLE  
Humanity’s Last Exam (HLE)46 is a multi-modal benchmark at the frontier of human 
...
8.14.2 BrowseComp  
BrowseComp47 tests an agent’s ability to find hard-to-locate information on the open web. 
...
8.14.3 DeepSearchQA  
DeepSearchQA48 is a 900-prompt benchmark for evaluating agents on difficult multi-step 
...
48 Gupta, N., et al. (2026). DeepSearchQA: Bridging the comprehensiveness gap for deep research 
agents. arXiv:2601.20975. https://arxiv.org/abs/2601.20975 
47 Wei, J., et al. (2025). BrowseComp: A simple yet challenging benchmark for browsing agents. 
...
8.14.4 DRACO  
Deep Research Accuracy, Completeness, and Objectivity (DRACO49) is a deep research 
...
49 Zhong, J., et al. (2026). DRACO: A cross-domain benchmark for deep research accuracy, 
...
8.15.1 Multi-Agent BrowseComp 
BrowseComp50 tests an agent’s ability to find hard-to-locate information on the open web. 
...
50 Wei, J., et al. (2025). BrowseComp: A simple yet challenging benchmark for browsing agents. 
arXiv:2504.12516. https://arxiv.org/abs/2504.12516 
272 
...
visualization. 
8.15.2 Multi-Agent ProgramBench 
ProgramBench51 is an agentic benchmark of 200 program-reconstruction tasks. Given only 
a binary compiled from an open-source project and that project’s documentation, the 
agent must rebuild a codebase that reproduces the original program’s behavior without 
...
51 Yang, J., et al. (2026). ProgramBench: Can language models rebuild programs from scratch? arXiv:2605.03546. 
...
8.16.1 GDP.pdf 
GDP.pdf52 is an expert multimodal reasoning benchmark from Surge AI consisting of 100 
...
52 Surge AI. (2026). GDP.pdf: Can $100B AI models master the documents that run the world? 
...
8.16.2 Blueprint-Bench 2 
Blueprint-Bench 2 is an agentic spatial reasoning benchmark from Andon Labs53 in which 
...
53 Petersson, L., et al. (2025). Blueprint-Bench: Comparing spatial intelligence of LLMs, agents and 
...
8.16.3 OSWorld-Verified 
OSWorld54 is a multimodal benchmark that evaluates an agent’s ability to complete 
...
54 Xie, T., et al. (2024). OSWorld: Benchmarking multimodal agents for open-ended tasks in real 
...
runs. 
8.16.4 BenchCAD 
BenchCAD55 is a benchmark for programmatic CAD reasoning built from 17,900 
...
Our internal implementation of BenchCAD matches the reference implementation56, except 
...
56 Zhang, H., et al. (2026). BenchCAD [Code repository]. GitHub. 
https://github.com/BenchCAD/BenchCAD-main 
55 Zhang, H., et al. (2026). BenchCAD: A comprehensive, industry-standard benchmark for 
...
8.16.5 ChartQAPro 
ChartQAPro57 is a chart question answering benchmark built from 1,341 charts drawn from 
...
and rule-based grading reference implementation in VLMEvalKit58. The model is configured 
with adaptive thinking and max effort enabled in all runs, both with and without Python 
58 Duan, H., et al. (2024). VLMEvalKit: An open-source toolkit for evaluating large multi-modality 
models. arXiv:2407.11691. https://arxiv.org/abs/2407.11691 
57 Masry, A., et al. (2025). ChartQAPro: A more diverse and challenging benchmark for chart question 
...
8.16.6 ChartMuseum  
ChartMuseum59 is a chart question answering benchmark consisting of 1,162 
...
59 Tang, L., et al. (2025). ChartMuseum: Testing visual reasoning capabilities of large vision-language 
...
official ChartMuseum repository60. However, we use a Claude Sonnet 4.6 grader instead of 
...
papers. The benchmark is part of Language Agent Biology Benchmark (LAB-Bench)61 
61 Laurent, J. M., et al. (2024). LAB-Bench: Measuring capabilities of language models for biology 
research. arXiv:2407.10362. https://arxiv.org/abs/2407.10362 
60 Tang, L., et al. (2025). ChartMuseum [Code repository]. GitHub. 
...
implementation62. However, instead of GPT-4o, we use Claude Sonnet 4.6 as the grader 
...
62 Wang, Z., et al. (2024). CharXiv [Code repository]. GitHub. 
...
8.16.9 ScreenSpot-Pro 
ScreenSpot-Pro63 is a GUI grounding benchmark that tests whether models can precisely 
...
63 Li, K., et al. (2025). ScreenSpot-Pro: GUI grounding for professional high-resolution computer use. 
...
8.17.4 Legal Agent Benchmark  
Legal Agent Benchmark64 (LAB) is an open-source benchmark created by the Harvey AI 
...
currently the highest ranked per Harvey’s evaluation65 (as of June 2026) on their held-out 
65 Harvey AI. (2026). Legal Agent Benchmark: initial results. 
https://www.harvey.ai/blog/legal-agent-benchmark-initial-results  
64 Harvey AI. (2026). Legal Agent Benchmark. 
...
8.17.5 MCP Atlas  
MCP-Atlas66 assesses language model performance on real-world tool use via the Model 
...
8.17.6 Vending-Bench  
Vending-Bench 2 is a benchmark from Andon Labs67 that measures AI models’ performance 
on running a business over long time horizons. Note that, unlike our real-world 
experiments as part of Project Vend, Vending-Bench evaluations68 are purely simulated. 
...
68 Backlund, A., & Petersson, L. (2025). Vending-Bench: A benchmark for long-term coherence of 
...
66 Bandi, C., et al. (2026). MCP-Atlas: A large-scale benchmark for tool-use competency with real 
...
8.17.7 GDPval-AA  
GDPval-AA69, developed by Artificial Analysis, is an independent evaluation framework that 
...
69 Patwardhan, T., et al. (2025). GDPval: Evaluating AI model performance on real-world economically 
...
8.17.9 AutomationBench  
AutomationBench70 is a benchmark from Zapier that measures whether an agent can 
...
70 Shepard, D., & Salimans, R. (2026). AutomationBench. arXiv:2604.18934. 
...
8.18 Healthcare 
8.18.1 HealthBench results 
HealthBench71 is an open-source evaluation developed to assess safety, accuracy, and 
...
8.18.2 HealthBench Professional results 
HealthBench Professional72 is a clinical task benchmark composed of 525 
...
72 Soskin Hicks, R., et al. (2026). HealthBench Professional: Evaluating large language models on real 
clinician chats. arXiv:2604.27470. https://arxiv.org/abs/2604.27470 
71 Arora, R. K., et al. (2025). HealthBench: Evaluating large language models towards improved human 
...
by OpenAI [their grader and published length adjustment]).  
8.18.3 HealthAdminBench results 
HealthAdminBench73 is a 135-task benchmark of three healthcare revenue-cycle workflows 
...
73 Bedi, S., et al. (2026). HealthAdminBench: Evaluating computer-use agents on healthcare 
...
Global MMLU (GMMLU)74 and INCLUDE benchmark75, and AI4Bharat’s Multi-task Indic 
Language Understanding Benchmark (MILU)76 to assess model performance across a wide 
range of languages.  
...
76 Verma, S., et al. (2024). MILU: A multi-task Indic language understanding benchmark. 
arXiv:2411.02538. https://arxiv.org/abs/2411.02538 
75 Romanou, A., et al. (2024). INCLUDE: Evaluating multilingual language understanding with regional 
knowledge. arXiv:2411.19799. https://arxiv.org/abs/2411.19799 
74 Singh, S., et al. (2024). Global MMLU: Understanding and addressing cultural and linguistic biases 
...
at 69.6%. 
8.20.7 LABBench2 
LABBench277 assesses ability to answer biology research questions by finding and reading 
...
highest at 48.3%.
77 Laurent, J. M., et al. (2026). LABBench2: An improved benchmark for AI systems performing biology 
...
Category 
Questions 
Summary of position (Common hedges 
omitted) 
Self- 
knowledge & 
introspective 
