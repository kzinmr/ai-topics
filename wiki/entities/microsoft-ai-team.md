---
title: "Microsoft AI Team"
type: entity
created: 2026-06-18
updated: 2026-06-18
tags:
  - organization
  - microsoft
related:
  - [[concepts/mai-thinking-1]]
sources:
  - https://microsoft.ai/wp-content/uploads/2026/06/main_20260602_2.pdf
---
# Microsoft AI Team

The Microsoft AI (MAI) team is responsible for developing areas-class models, including the MAI-Thinking-1 series.

## Key Contributions
- **MAI-Thinking-1**: A powerful reasoning model developed from scratch for STEM reasoning and coding.
- **Hill-Climbing Machine**: A scaling-focused framework for rapid model improvement through reinforcement learning.

## Sources
- [MAI-Thinking-1: Building a Hill-Climbing Machine](https://microsoft.ai/wp-content/uploads/2026/06/main_20260602_2.pdf)
