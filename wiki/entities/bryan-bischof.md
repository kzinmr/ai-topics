...
Bryan appeared on Episode 2 of *Show Us Your (Agent) Skills* alongside his Theory Ventures colleague [[entities/tomasz-tunguz]], as well as [[entities/hilary-mason]] and [[entities/eric-ma]]. His segment (approximately 1:20:16 to 1:47:15, with a return at 2:14:18 for live BBplot Eval) was the third demo of the episode and focused on **BBplot**, a grammar of graphics for agents.

### BBplot: A Grammar of Graphics for the Agent Era

Bryan debuted BBplot publicly for the first time on the show. The core insight: existing charting libraries (matplotlib, ggplot) are designed for human programmers, but agents need different primitives.

> "What if all the charts you want to make you were making through human-agent conversation? What would the stack of fundamental ideas from grammar of graphics look like in the agent era?"

**Key design principles from his BBplot Manifesto:**

1. **Package context with the content**: Every chart output includes the data, config, and scene graph alongside the rendered image. "You are providing lots of context about what you want built. You either often lose that context or you package it as a separate thing that kind of gets lost or forgotten. What if you just force it to be in the same place?"

2. **Ambiguity yields variance — always generate 3 variations**: Drawing from Midjourney's four-image pattern, Bryan argued that single-pass generation locks into one interpretation of ambiguous specs. Generating multiple variations surfaces the ambiguity and gives the human real choice. "I'm top K. K does not need to equal one. Like, you're allowed to let K equal five."

3. **Scene graph for layout-aware charting**: Every chart emits a scene graph — a structured description of what's on the chart, where, and how elements relate. This solves the classic agent problem of overlapping elements ("don't make the legend overlap the data"). "Agents really struggle to understand those kind of things. Even VLMs struggle to do image understanding in a way that can easily defrag where the legend position is. This helps."

4. **Every feature must be demonstrated in the gallery**: Since BBplot's primary consumers are agents (not humans), documentation means demonstrated examples. Every feature requires a gallery entry with both the spec and the rendered image. Agents learn by example.

5. **The DSL should be optimized for agents**: "As a human, I don't actually know what agents want... If someone tells you 'I know what agents want,' you should be immediately suspicious."

### BBplot Eval: Eval-Driven Feature Development

The companion project, **BBplot Eval**, is a separate repository designed to prevent agents from "cheating":

- **Separate repo, pinned version**: The eval agent cannot look at BBplot's source code — only the documented gallery examples
- **Eval cards**: Each target chart is an "eval card" with prompts, failure funnels, and predictions on how the agent could screw up
- **Feature requests from eval failures, not human wishlists**: "The feature requests are not allowed to mention specific evals. But everything that is built in BBplot is not generated by me, a human. It is generated by failures at the evals."
- **Builder agent workflow**: BBplot Eval's builder agent tries to pass evals → fails → writes failure report → BBplot's builder agent implements the fix → version released → ratchet (never regress)
- **"The goal is not to score high on the benchmark. The goal is to generate information about gaps."**

### Self-Updating Skills

Bryan revealed a meta-skill pattern where agents update their own skills:

> "I have a workflow built... multiple workflows actually. And these are things that have been written by the agent because I've complained. I said, 'No, you screwed up again. Make yourself a note' and then I review the note."

This creates a self-improving loop: agent fails → writes workflow/skill note → human reviews → agent improves. He also noted the trap of agents over-indexing on specifics when writing workflows — they must be prompted to generalize ("literally pull out file names from the current workflow").

### Live BBplot Eval Demo

During Tomasz Tunguz's segment, Bryan ran a live BBplot Eval against a new chart (Robert De Niro's Rotten Tomatoes scores by age). The agent:

1. Received the reference image and attempted to recreate it with BBplot
2. Self-evaluated: "no shaded backgrounds, no curved arrow annotations, the hand-drawn aesthetic, and the label crowding" — detected via Scene Graph warnings
3. Found a gallery example with shaded regions and iterated
4. The first attempt was not promoted because evaluation criteria weren't satisfied — demonstrating the ratchet

### Tool Preferences and Challenges

Bryan shared his development setup: Cursor as primary IDE (tried Codex but abandoned it — "incapable of using a local file system and constantly breaks with image rendering"), Opus as daily driver, and a local DuckDB instance publishing all eval traces for analysis.
...
- **[[entities/will-mcgugan]]** — Fellow Weights & Biases community figure; both interested in developer experience and tooling
