...
- **Always-On Agent**: DGX Spark's 24/7 continuous operation design aligns with Hermes' always-on execution model (cron, gateway, self-improving loop)
- **Large Model Compatibility**: 128GB unified memory enables all-day execution of 120B MoE models (necessary for Hermes' advanced reasoning tasks)
- **Qwen 3.6 Optimization**: Qwen 3.6 35B (120B-class intelligence at 20GB) is the recommended model on DGX Spark. Qwen 3.6 27B (comparable to 400B-class) is also high-performance
- **Official Playbook**: NVIDIA's official [Hermes DGX Spark playbook](https://github.com/NVIDIA/dgx-spark-playbooks) provides setup instructions
- **Harness Engineering**: NVIDIA's blog explicitly states that "Hermes achieves superior results with the same model" — the framework design as an active orchestration layer maximizes DGX Spark's hardware performance

**Recommended local runtimes** with Hermes on DGX Spark:
- **llama.cpp** — CPU/GPU hybrid inference
- **Ollama** — Simplest path (Hermes supports out of the box)
- **LM Studio** — GUI-based model management (Hermes supports out of the box)
