...
## Pricing Test Incident (Apr 2026)

### What Happened
- Claude Code was **quietly removed from the $20 Pro plan** (including Max plan users), moved exclusively to the $100/month Max plan.
- Critics called it "borderline suicidal" for a company built on coding reputation.
- **Reappeared within 24 hours** — confirmed to be a deliberate pricing elasticity test (a "fake door" probe).

### Economics
- Inference costs for Claude Code are severe; Anthropic was reallocating compute to higher-LTV customers to improve unit economics.
- Every frontier AI lab faces the same capacity constraint problem.

### Fallout
- OpenAI's **Rohan Varma** mocked the move publicly, contrasting it with OpenAI's approach.
- The incident eroded developer trust — Anthropic markets safety and integrity, but a silent pricing test on the entry tier was seen as disingenuous.
- Highlighted the fundamental tension: Claude Code is the most popular AI coding agent by usage (36% of all Claude interactions, 79% automated), but its compute costs make it hard to subsidize at Pro-tier pricing.
...
Claude Design (released April 2026) creates a direct handoff pipeline to Claude Code. When a design is complete, Claude Design packages everything into a handoff bundle that Claude Code can receive with a single instruction. This creates a **design-to-code** workflow:

1. Designer creates visual mockup in Claude Design
2. Claude Design generates handoff bundle (assets, specs, component structure)
3. Claude Code receives bundle and begins implementation
4. Iterative refinement between designer and coder via shared context

This integration blurs the boundary between design and engineering workflows, enabling PMs and designers to prototype and then hand off directly to an AI coding agent without traditional design-review handoff cycles.
