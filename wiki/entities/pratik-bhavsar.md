...
### Continual Learning & Agent Improvement

- **Continual Learning Without Evals Is Just Drift** (Apr 2026) — Critiques Harrison Chase's three-layer continual learning framework (model, harness, context) for treating evaluation as a given rather than the core infrastructure. Argues that **evaluated traces**, not raw traces, are the primitive that powers reliable learning. Introduces a six-step cycle: Observe → Evaluate → Diagnose → Intervene → Verify → Repeat. Connects runtime guardrails to learning infrastructure: every Deny/Steer produces structured evaluation signals that feed back into the learning loop. Source: [[raw/articles/2026-04-08_pratik-bhavsar-continual-learning-without-evals.md]]
