...
tags: [person]
sources: [raw/articles/nesbitt.io--2026-08-04-brew-install-actions-checkout-html--7762f317.md, raw/articles/nesbitt.io--2026-08-13-supplier-security-questionnaire-html--5f6fc9cf.md]
created: 2026-04-24
updated: 2026-08-14
type: entity
---
...
### GitHub Actions Registry from Homebrew Parts (August 2026)

Published August 4, 2026. Nesbitt assembles a **GitHub Actions registry from Homebrew's parts**, exploiting the fact that immutable actions and Homebrew bottles are now the same kind of object: OCI artifacts on ghcr.io. It is a concrete answer to the gap he identified in December — `uses:` is a package manager with no lockfile, no integrity hashes and no transitive visibility — and to the run of incidents that followed ([[raw/articles/nesbitt.io--2026-04-28-github-actions-is-the-weakest-link-html--0c77c59b.md]]).

**The gap neither roadmap item closes**: GitHub's 2026 security roadmap committed to a lockfile, now in preview as **gh-actions-lock**, and made immutable actions the preferred resolution path. Neither change adds any review between an action author tagging a release and the runner executing it. Homebrew has run that kind of curated index for fifteen years and, as of the immutable-actions rollout, stores its artifacts as OCI manifests on ghcr.io alongside the actions themselves.

**Shared storage**: `actions/publish-immutable-action` tars the action directory, pushes it as a layer with `artifactType: application/vnd.github.actions.package.v1+json`, attaches a sigstore bundle through the OCI referrers API, and tags the manifest with the semver — after which `actions/checkout@4.2.2` resolves through `pkg.actions.githubusercontent.com` instead of the git tarball. `brew pr-pull` pushes bottles under the same manifest schema at `ghcr.io/homebrew/core/<name>`, with a `com.github.package.type: homebrew_bottle` annotation and a sigstore attestation that `brew verify` checks against Homebrew's CI identity. `crane manifest ghcr.io/homebrew/core/jq:1.7.1` and `crane manifest ghcr.io/actions/checkout:4.2.2` return the same document type.

**The December comparison table, filled in**: the December post marked Actions ✗ on integrity hashes, transitive visibility, dependency-tree inspection and immutable versions; homebrew-core provides all four for its 8,400 formulae **through the index rather than the storage** — each formula pins a source URL to a sha256, declares dependencies that `brew deps --tree` can walk, passes `brew audit` and human review on every change, gets autobumped by `livecheck` when upstream tags a release, and can carry `deprecate!` or `disable!` when it shouldn't be installed.

**A tap of actions**: the index can be a tap, each formula pinning an action tarball by SHA-256 — `desc`, `homepage`, `url` pinned to the release tarball, `sha256`, `license`, a `livecheck` with `:github_latest`, and a `def install` that copies the tree into the prefix. For a JavaScript action that ships a built `dist/` in its release tarball, that's sufficient: `brew audit` and `brew verify` apply as they would to any formula, and `brew bump-formula-pr` opens a reviewed PR when checkout tags v4.2.3. Everything above `def install` is already static data — and Homebrew is migrating install hooks to declarative steps so that bottle and cask installs need no Ruby evaluation at all, at which point an actions tap could be `.json` files with no code execution on install.

**The transitive problem is specific to composite actions**, whose `action.yml` carries its own `uses:` lines that the runner re-resolves at execution time regardless of how the outer action was pinned. In a formula those become `depends_on` entries plus an `inreplace` at build time: a composite calling `actions/cache@v4` gets `depends_on "actions-cache"` and its install rewrites `uses: actions/cache@v4` to `uses: ./.brew-actions/actions-cache`. The resulting bottle has no floating refs left in it, `brew deps --tree` prints the transitive graph that no runner command exposes today, and the tap's git log records which `actions-cache` revision the composite was built against. Moving that pin requires a reviewed PR; an action author cannot change it with `git tag -f` in someone else's repository.

> *"Every incident in the weakest-link post would have required a reviewed change to that index before reaching downstream users, where an npm-style per-project lockfile would only have reduced the number of downstream repositories exposed. A workflow that pins @v4 today has already delegated the version decision to whoever can push a tag to the action repo, and a tap moves that delegation to a reviewer instead."*

The tap also matches Homebrew's rolling-release design — `Brewfile.lock.json` was removed in November 2024 and per-project pinning is currently out of scope. Nesbitt would like the lockfile back this year; until it returns, a workflow needing stricter reproducibility than the tap's HEAD can pin the tap itself to a commit.

**Beyond pinning**: an `audit_formula` extension would run **zizmor** over the extracted `action.yml` and reject anything tripping `dangerous-triggers` or `template-injection`, plus composites whose internal `uses:` lines aren't fully covered by `depends_on`. The Marketplace's "verified creator" badge checks the publisher's identity and nothing about the action's contents, so a static-analysis gate at index time would be new. Bottles built from the tap are attested by the tap's CI the same way homebrew-core bottles are, and each formula's `url` points at a GitHub repository — the input `brew vulns` already keys OSV lookups on — so an advisory against `actions/download-artifact` surfaces through the same path as one against `openssl`.

**Getting the runner to use it**: the runner has three `ActionSourceType` values (repository, container registry, script) and none of them is "an installed package on disk". Integration points, in increasing cost:

- **`uses: ./path`** anchored at `$GITHUB_WORKSPACE`: the prototype tap at `andrew/homebrew-actions` packages checkout, cache, pre-commit/action and first-interaction; its setup action runs `brew bundle --file .github/Actionfile` and copies each keg into `./.brew-actions/<name>`. Still costs one round-trip to fetch the setup action itself the old way, and checkout stays on a plain SHA pin because it runs before the workspace has anything to `uses: ./` from.
- **The `$/` prefix** (runner 2.336.0) anchors at the repository containing the defining file, resolved at the running commit — readable before any step has run, and valid for reusable workflows. But inside a composite loaded via `uses: ./path` it still resolves against the workflow's repository rather than the copied directory (an earlier prototype rewrote composite refs to `$/../actions-cache` and the runner attempted to fetch `andrew/homebrew-actions/../actions-cache@<sha>`), so the `.brew-actions` destination has to be baked into the formula's `inreplace`.
- **A fourth `ActionSourceType`** reading a formula's JSON from formulae.brew.sh (or any tap's API endpoint), verifying the bottle attestation, and extracting to `_actions/` — this affects self-hosted runners and compatible forks such as `act` and Forgejo's runner, with no GitHub backend changes. `ACTIONS_RUNNER_ACTION_ARCHIVE_CACHE` already gives self-hosted runners an offline mirror, but cannot override `@v4` resolution. Reusable workflows referenced across repositories have no local-path form even with `$/`, and Docker actions already resolve through a container registry.

**Built in**: GitHub could integrate the tap into its resolver and leave `uses: actions/checkout@v4` unchanged — an organisation or repository setting alongside the existing action allowlists would name the index the resolver consults, mapping `actions/checkout` to a bottle manifest digest instead of a git ref. The runner pulls the layer from ghcr.io, verifies the attestation against the tap's CI identity, and extracts to `_actions/`. Because composite internal refs were rewritten at bottling time, the index response could carry the resolved dependency closure, making the whole transitive tree one index query plus N content-addressed blob fetches; re-running a job against an unchanged tap commit would produce identical bytes. Switching an organisation from GitHub's default index to a community tap or an internal one with stricter audit rules becomes a settings change — roughly the choice apt users make between Debian stable and a private mirror. The alternative to a settings toggle is package URLs in the `uses:` line itself (`pkg:brew/actions-setup-python`, or `pkg:oci/...@sha256:...` for a direct digest pin), with org policy allowlisting which types and namespaces are permitted. Either way Homebrew's role is producing one index in the formulae.brew.sh JSON schema — and once the declarative-install work lands, the tap and the index are the same JSON, so any other curator could produce a compatible one without going near Homebrew's Ruby.

**Prototype**: draft formulae for the 70 actions in ecosyste.ms' current critical set — 55 JavaScript actions, nine composites and six Docker actions — with all 71 declared `pre`/`main`/`post` entrypoints present in the archives. Seven composites declare external actions (seven dependency edges into five repositories, four of which are already in the 70; the two Gradle compatibility actions depend on subdirectories of gradle/actions, so closing the first-level graph takes one extra formula — 71 in all). Running zizmor 1.28.0 over the extracted `action.yml` files, as the audit extension would, reported **61 findings across seven of the 70** — 45 high-severity, high-confidence findings in five actions (40 template-injection and five unpinned-uses), and five of the six Docker actions reference their image or Dockerfile base image by tag rather than digest. The generator also hit ecosyste.ms data issues (two conda-forge packages whose latest release is recorded as `master` with now-404 codeload URLs though the stored commit SHAs still fetch, conda-forge/webservices-dispatch-action classified as composite when the pinned source is a Docker action, and Platane/snk with no license file for `brew audit` to accept).

Codeberg, Forgejo and Gitea each already maintain an `actions` org (codeberg.org/actions, code.forgejo.org/actions, gitea.com/actions) mirroring a hand-picked subset of upstream actions — the curated index a tap would produce, maintained by hand — and the Forgejo maintainers have been vocal about wanting something better than inheriting GitHub's resolution model. Their runners (and Gitea's act_runner) implement the same `uses:` semantics with no closed server-side resolution call, so the setup-step path works on them today and either built-in form could land there without a GitHub backend change.

Source: [[raw/articles/nesbitt.io--2026-08-04-brew-install-actions-checkout-html--7762f317.md]]
Related: [[concepts/package-management]], [[concepts/software-supply-chain-security]], [[concepts/dependency-graphs]]

---

### Supplier Security Questionnaire — Satire (August 13, 2026)

Published August 13, 2026. A satirical **annual supplier security assessment** in the voice of an enterprise "Third-Party Risk Management" function writing to an OSS maintainer at "[address extracted from package metadata]." The questionnaire inverts the modern open-source supply chain relationship: a package maintainer — often an individual with no procurement contact, no SOC 2, no revenue — is asked to self-certify against demands written for a Fortune 500 vendor, with *"Blank responses are scored as a fail"* and *"Estimated time to complete: 20 minutes."*

**The absurdity is layered through seven sections:**

- **Section 2 (Security)**: requires SOC 2 Type II / ISO 27001, pre-employment background screening for "all persons with commit access" — including "persons cohabiting at that facility who are not authorised to commit" — plus physical access controls at the facility where the Component is developed.
- **Section 3 (Secure Development)**: asks the sole maintainer to "describe the controls that prevent [you]" from inserting malicious code, demands Critical/4h remediation timescales, and requires confirmation that all contributors completed annual secure software development training.
- **Section 4 (Business Continuity)**: quantifies bus factor with surgical precision — commute-by-bus questions, key-person life insurance policies "naming ourselves as beneficiary," and monitoring of public statements for "intention to reduce their involvement."
- **Section 5 (Artificial Intelligence)** — the AI-relevant core:
  - *5.1* — Was any portion of the Component generated by an AI system, LLM, or code completion tool? "Please identify each model and version and attach the provider's terms of service as in force on the date of generation."
  - *5.3* — "Describe the controls preventing prompt injection against any AI systems used in the development or maintenance of the Component."
  ...
  - *5.6* — "Does the supplier consent to the appointment of an autonomous agent... as an additional maintainer of the Component, for the purpose of meeting the timescales in 3.3?"
- **Section 6 (Legal & Export)**: demands a warranty "free of defects, fit for purpose, and of merchantable quality" notwithstanding the license disclaimer, plus the residential address for service of legal process.
- **Section 7 (Financial)**: requests travel & expenses policy, USD 5M cyber liability insurance naming the customer as additional insured, and a promise of maintenance through 2034 with 180 days' notice of any future charge.

**The punchlines**: *"Submitted questionnaires are reviewed in the order received. Current review time is approximately nine to twelve months"*; *"In line with our memory-safety policy, where the Component is implemented in Rust, Sections 2 and 3 are scored as satisfactory regardless of the responses given. All questions remain mandatory"*; *"Removal would require significant engineering effort on our part and is therefore unlikely to be approved, but the finding will remain open against the supplier indefinitely"*; and *"There is no compensation associated with the completion of this questionnaire."*

The piece is a direct satirical counterpart to his analytical work on [[concepts/ai-supply-chain-security]] and [[concepts/software-supply-chain-security]] — where his essays document how enterprises fail to secure OSS, this one documents how enterprise risk frameworks would fail if applied to OSS as written. The AI governance questions (5.1-5.6) satirize the emerging category of AI-supply-chain due diligence demands that treat LLM-generated code, prompt injection, and autonomous-agent contributions as auditable vendor obligations.

Source: [[raw/articles/nesbitt.io--2026-08-13-supplier-security-questionnaire-html--5f6fc9cf.md]]
Related: [[concepts/ai-supply-chain-security]], [[concepts/software-supply-chain-security]], [[concepts/agentic-engineering]]
...
- raw/articles/nesbitt.io--2026-08-13-supplier-security-questionnaire-html--5f6fc9cf.md
