...
**Part 2 — The Bugfix** (July 2026): While writing up Part 1, Thomas ran the draft past his "editorial board" of AIs (ChatGPT), which highlighted a bug in the eval code: `model.state_dict()` returns **references** to parameters, not copies — so `load_state_dict` was a no-op and models were evaluated in a partially-trained state. The eval was also only using the first 5 batches of the test split. Fixing both changed the baseline numbers (re-ordering how his own models did) but **did not change the important result**: OpenAI's weights still beat his on instruction-following. The evaluation uses GPT 5.5 as LLM-as-a-judge, showing all models' responses for each question in the same prompt, shuffled per run. Source: raw/articles/gilesthomas.com--2026-07-why-do-openai-gpt2-weights-beat-mine-2-the-bugfix--d4435f0c.md

**Part 3 — Testing Overtraining** (July 2026): Tests the most plausible theory from Part 1 — that OpenAI's GPT-2 weights were "overtrained" (trained on more than the Chinchilla-optimal ~20 tokens/parameter) and that this explains their instruction-following advantage. Thomas deliberately overtrained his own models (6.4B tokens, plus a 2-epoch variant), with results: the 6.4B-token model reached test loss **3.325897** — beating OpenAI small (3.499677) and approaching OpenAI medium (3.231442); the 2-epoch model hit 3.324953. **Conclusion: overtraining didn't help much, if at all** — the mystery of OpenAI's weights remains. The post also documents practical JAX training learnings (CUDA stream capture errors, etc.). Source: raw/articles/gilesthomas.com--2026-07-why-do-openai-gpt2-weights-beat-mine-3-overtraining--fae7948d.md
