---
title: "Jack Clark"
created: 2026-05-05
updated: 2026-05-27
type: entity
tags: [person, company, anthropic, safety, prediction, governance]
sources: [raw/articles/2026-05-04_import-ai-455-automating-ai-research.md, raw/newsletters/2026-05-26-import-ai-458-reckoning-with-the-future-and-a-singularity-story.md]
---

# Jack Clark

## Overview

Jack Clark is a co-founder of **[[entities/anthropic]]** and the former Policy Director at OpenAI. He currently serves as Anthropic's Head of Policy and writes the influential weekly newsletter **Import AI**, one of the longest-running AI-focused newsletters covering research, policy, and industry trends. His writing is known for combining technical depth with clear-eyed strategic forecasting.

## Background

- **Co-founder, Anthropic** (2021–present) — Head of Policy
- **Policy Director, OpenAI** (2016–2021)
- **Import AI newsletter** — Weekly analysis of AI research and industry developments
- Previously a journalist covering AI at Bloomberg and The Register

## Core Contributions

### Import AI Newsletter
Import AI is one of the most widely-read AI newsletters in the world, known for:
- Deep analysis of AI research papers and industry developments
- Probabilistic forecasting of AI capabilities and risks
- Early identification of emerging trends (recursive self-improvement, automated alignment)
- Distinctive blend of technical literacy and policy awareness

### AI Policy Leadership
- Key architect of Anthropic's approach to responsible scaling and frontier model governance
- Advocate for structured evaluation-based regulatory frameworks rather than blanket restrictions
- Contributor to the U.S. AI Safety Institute (AISI) discussions

## Key Ideas

### Recursive Self-Improvement (May 2026)
In Import AI 455, Clark made a landmark prediction: **60%+ probability of autonomous AI R&D by end 2028**, where AI systems build their own successors without human involvement. Key evidence:
- **SWE-Bench saturation**: Claude Mythos Preview at 93.9%, up from 2% (late 2023)
- **Time horizons**: METR tasks expanded from 30 seconds (GPT-3.5) to ~12 hours (Opus 4.6)
- **Training optimization**: Claude Mythos Preview achieved 52x speedup on CPU-only training
- **Compounding alignment risk**: 99.9% alignment accuracy drops to 60.5% over 500 generations

See: [[concepts/recursive-self-improvement]]

### Alignment as Compound Risk
Clark frames AI alignment not as a binary problem but as a compounding one — even near-perfect alignment decays exponentially across recursive self-improvement generations. This makes automated alignment research (AAR) both critical and inherently risky.

### The Amdahl's Law Economy
As AI accelerates digital work, physical-world bottlenecks (drug trials, manufacturing) become the primary constraints on economic growth — what Clark calls "Amdahl's Law for the economy."


### Import AI 458: Reckoning with the Future (May 2026)

In Import AI 458, Clark wrote a deep personal essay on how exponential AI progress forces individuals to make an unavoidable choice: "explore the future or retreat from the present." The issue combines a lecture essay (Parts 1-3) with a fictional story. Core themes:

- **Epoch AI Capabilities Index**: Performance scales predictably with compute and data — a framework for forecasting AI advancement
- **Personal AI journey**: From dealing with typos in 2023 to having an "deep intellectual partner" by 2026
- **Import AI as superpower**: What started as a hobby newsletter became a powerful analysis tool through AI-assisted synthesis
- **Individual choice**: Clark argues that continued exponential progress removes the option of neutrality — everyone must decide their relationship with accelerating AI

See [[raw/newsletters/2026-05-26-import-ai-458-reckoning-with-the-future-and-a-singularity-story.md]] for full text.

## Writing Style
Clark writes with a distinctive combination of technical precision and literary flair. His newsletter intros often set context through historical analogies or science fiction references before diving into data. He is notably willing to make concrete, falsifiable predictions with explicit probability estimates.

## Related

- [[entities/anthropic]] — Co-founder and Head of Policy
- [[entities/dario-amodei]] — Anthropic CEO, co-founder
- [[concepts/recursive-self-improvement]] — Core concept advanced in Import AI 455
- [[concepts/ai-safety]] — Policy and alignment work
- [[concepts/automated-alignment-research]] — Anthropic's AAR initiative
- [[concepts/model-distillation]] — Related policy debates

## Links

- **Import AI**: [importai.net](https://importai.net)
- **Anthropic**: [anthropic.com](https://anthropic.com)
- **X/Twitter**: [@jackclarkSF](https://x.com/jackclarkSF)
