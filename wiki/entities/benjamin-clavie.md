...
**Tsukuba IR Talk (2026-05-20):** 「ColBERT and Late Interaction Retrieval: Why, How, and What Next?」でClaviéはIRパラダイムの体系的な俯瞰を提供。重要な洞察：Single-Vectorの限界はモデルそのものではなく**スコアリング機構（cosine similarity）**にあること、MaxSimのMaxコンポーネントが性能の鍵（SumSim/AvgSimでは崩壊）、ColBERTはシングルベクトルより1桁少ないデータで同等性能（データ効率性）、BrowseComp-Plusで0.13B ColBERTが8Bシングルベクトルモデルを上回る。未解決問題として：oracle gap（最良の検索でも~12%のタスクが誤り）、MaxSimの命令追従崩壊、大規模対照事前学習の遅れ（2026年4月に初出、シングルベクトルでは5年前から標準）。[[raw/articles/2026-05-20_clavie-tsukuba-ir-talk-colbert-late-interaction|slides]]
...
- **[[concepts/jeremy-howard]]** — Answer.AI共同創設者。Claviéが2024年に参加。fast.ai哲学の影響を強く受ける
- **[[entities/omar-khattab]]** — ColBERT/DSPy発明者（Stanford）。ClaviéのColBERT研究の直接的インスピレーション
- **[[concepts/benjamin-warner]]** — ModernBERT共同リード、fastkmeans共著者
- **[[concepts/antoine-chaffin]]** — ModernBERT共同リード、PyLate開発者
- **[[concepts/makoto-kato]]** — ClaviéのPhD指導教員（NII Tokyo/Kasys lab）
- **[[concepts/griffin-adams]]** — ColBERT token pooling共著者
- **[[concepts/rikiya-takehi]]** — mxbai-edge-colbert-v0主著者（Waseda Univ/Mixedbread intern）
- **[[concepts/xianming-li]]** — Mixedbread/Wholembed v3共著者、ECIR 2026 LIR Workshop co-organizer
- **[[tom-aarsen]]** — Hugging Face、PyLateコントリビューター、ECIR 2026 LIR Workshop co-organizer
