...
@sairahul1（[[entities/sairahul1|Rahul]]）による Claude Code を使った個人開発者向け Dark Factory 実践ガイド。7つの特化エージェントによるパイプラインで、vibe coding の限界を克服する。

### 7エージェント構成

| # | Agent | 役割 | ツール制限 |
|---|-------|------|-----------|
| 1 | **Codebase Researcher** | コードベース調査・パターン文書化 | Read, Grep, Glob（読み取り専用） |
| 2 | **Story Writer** | ユーザーストーリー・受入基準の作成 | Read（読み取り専用） |
| 3 | **Spec Writer** | 技術設計書（青写真）の作成 | Read, Grep, Glob（読み取り専用） |
| 4 | **Backend Builder** | API・サービス・DB・単体テスト実装 | Read, Edit, Write, Bash（バックエンドのみ） |
| 5 | **Frontend Builder** | コンポーネント・ページ・UIテスト実装 | Read, Edit, Write, Bash（フロントエンドのみ） |
| 6 | **Test Verifier** | 受入基準に対する受入テスト作成 | Read, Edit, Write（テストファイル）, Bash |
| 7 | **Implementation Validator** | 実装と仕様の差分レポート（修正しない） | Read, Grep, Glob（読み取り専用） |

### パイプライン設計の要点

- **3回の人間チェックポイント**: ストーリー承認 → 設計書承認 → PRレビュー（それ以外は完全自動）
- **CLAUDE.md の重要性**: 100-300行のリポジトリルートMarkdown — AIが間違えるたびにルールを追加し、チームの集合知として進化
- **コンテキスト隔離**: 各エージェントは必要な情報だけを持つクリーンなコンテキストウィンドウで動作 — 誤った前提の伝播を防止
- **Context Drift ルール**: 小さなタイプミスはインライン修正、誤ったアーキテクチャ前提は会話ごと破棄して再スタート
- **2-3時間でセットアップ可能**: 個人開発者が週末に構築できる実用レベルのファクトリー

### StrongDMとの比較

| 側面 | StrongDM Attractor | @sairahul1 7-Agent Factory |
|------|-------------------|--------------------------|
| 対象 | チーム（3名） | 個人開発者 |
| コードレビュー | 人間は一切見ない | バリデーターが自動チェック、人間はPRのみ |
| テスト戦略 | Digital Twin Universe | 受入テスト + バリデーター |
| セットアップ | 大規模インフラ（cxdb, attractor） | 2-3時間・CLAUDE.md + 7エージェント定義 |
| 自律性レベル | Level 5（完全自律目標） | Level 4（人間が監督する協調チーム） |
...
- [[concepts/openai-symphony]] — Level 4を実現するオーケストレーター
- [[concepts/anthropic-managed-agents]] — Level 4のプラットフォーム基盤
- [[concepts/harness-engineering]] — 全レベルの基礎となる実行環境設計
- [[entities/sairahul1]] — 7-Agent Factory の考案者
