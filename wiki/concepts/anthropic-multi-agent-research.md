...
### Agent-Context 双対性 — IRのクエリ-文書双対性との相同

さらに一歩抽象化すると、エージェント-コンテキスト関係には **情報検索 (IR) におけるクエリ-文書双対性** と相同な構造が現れる。

| IRの双対性 | Agent-Contextの双対性 |
|---|---|
| クエリ = 短い情報要求 | サブエージェントタスク = 短い調査指示 |
| 文書 = クエリへの完全な回答 | サブエージェント結果 = タスクへの完全な回答 |
| 両者は**同じ意味空間**に射影される | タスクも結果も**同じトークン空間**で表現 |
| クエリ ≒ 短い文書、文書 ≒ 長いクエリ | objective ≒ 要約された結果、結果 ≒ 展開されたobjective |
| Pseudo-relevance feedback で境界融解 | Interleaved thinking で評価→再探索（relevance feedback） |
| ランキング関数がクエリと文書の適合を判定 | リードエージェントがタスクと結果の適合を判定・統合 |

この双対性の核心：「タスク指示」と「探索結果」は本質的に**同じ意味的対象の異なる粒度での表現**であり、どちらもLLMのトークン空間に射影される。サブエージェントはタスク（クエリ）をコンテキスト（文書）に展開し、リードエージェントは文書を圧縮してクエリを更新する — IRのpseudo-relevance feedbackと同じフィードバックループ。

> Anthropic自身「search is compression」と述べているが、これはまさに双対性の言い換えである。文書空間での探索結果を、クエリ空間での新たな判断に圧縮する — IRがクエリ→文書→適合フィードバック→クエリ拡張のループを回すのと同じ構造が、エージェントシステムでは objective→探索→interleaved thinking→objective更新 として現れる。
