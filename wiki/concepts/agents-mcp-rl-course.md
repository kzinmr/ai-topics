...
sources:
  ...
  - raw/articles/2025-04-14_corbt_art-trainer-new-rl-trainer.md
  - raw/articles/2025-04-29_corbt_art-e-mail-agent.md
...
### Kyle Corbitt Blog Series: Intellectual Background Before ART Development (Oct 2024 – Jan 2025)

A blog series by Kyle Corbitt tracing the evolution of his thinking from before and during OpenPipe/ART development — from fine-tuning through RLHF, RFT, and task classification.

- **[A Founder's Guide to AI Fine-Tuning](https://corbt.com/posts/a-founder-s-guide-to-ai-fine-tuning)** (2024-10-11) — Decision framework for fine-tuning: when to fine-tune (after exhausting prompt engineering, few-shot, and RAG), data preparation (quality over quantity, representativeness, consistency, deduplication), training process (start small, hold out a test set, iterate on data), and common pitfalls (overfitting, data contamination, evaluation neglect, premature fine-tuning). [[raw/articles/2024-10-11_corbt_founders-guide-to-fine-tuning]]
- **[RLHF Part 1: Hacker News Ranking](https://corbt.com/posts/hacker-news-rlhf-part-1)** (2024-10-28) — Practical case study applying RLHF to HN content ranking: prompt engineering → feature engineering → RLHF pipeline (500 pairwise comparisons, Bradley-Terry loss reward model, 78% accuracy). CTR improved from 25% to 60%. **Key lesson:** LLMs are good feature extractors but weak rankers; small amounts of preference data yield large improvements; feature engineering > model architecture. [[raw/articles/2024-10-28_corbt_hacker-news-rlhf-part1]]
- **[Analyzing OpenAI's RFT](https://corbt.com/posts/openai-rft)** (2024-12-30) — Technical analysis of OpenAI's Reinforcement Fine-Tuning (RFT). Designed for tasks with verifiable outputs using PPO + partial credit grading function (dense reward), trainable with small datasets (tens of examples). RFT applicability: task is difficult, output is verifiable, labeled data is scarce. RFT → SFT distillation pipeline (50 RFT → 20K SFT → 5M inferences). **Open-source RFT implementation for QwQ is also in progress.** [[raw/articles/2024-12-30_corbt_openai-reinforcement-fine-tuning]]
- **[Deterministic vs Freeform Tasks](https://corbt.com/posts/deterministic-vs-freeform-tasks)** (2025-01-14) — Framework classifying tasks as deterministic (single correct answer: classification, information extraction) vs freeform (multiple valid answers: summarization, chat). OpenPipe dataset analysis: 63% freeform / 37% deterministic (reversed for high-usage tasks). **Temperature:** deterministic=0, freeform=0.7-1.0. **Evaluation:** deterministic uses golden datasets, freeform uses vibe checks / LLM-as-judge / user feedback. **Fine-tuning:** deterministic → RFT + small model distillation, freeform → DPO / RLHF. **This taxonomy forms the theoretical foundation of the course's evaluation lectures.** [[raw/articles/2025-01-14_corbt_deterministic-vs-freeform-tasks]]

### Kyle Corbitt Blog Series: ART/RL Implementation and Practice (Apr 2025 – Jul 2025)

A practical blog series spanning from the ART framework announcement to the RULER release, directly relevant as supplementary material for the course's Lesson 4 (RL) and beyond.

- **[ART Trainer: A New RL Trainer for Agents](https://corbt.com/posts/art-trainer-a-new-rl-trainer-for-agents)** (2025-04-14) — ART framework announcement. Solves three limitations of existing trainers: no multi-turn support, GPU inefficiency, and difficult integration with existing codebases. **Architecture:** Frontend/Backend separation (local rollout + cloud GPU), OpenAI-compatible inference endpoint (vLLM), KV cache offloaded to CPU enabling 7B training on Colab's free tier. **Initial results:** 7B beats GPT-4o (Tic-tac-toe), 14B beats frontier models (Temporal Clue). [[raw/articles/2025-04-14_corbt_art-trainer-new-rl-trainer]]
- **[ART·E: Email Research Agent That Beats o3](https://corbt.com/posts/art-e-mail-agent)** (2025-04-29) — ART validation case study. Synthetic QA generation on Enron email dataset → GRPO training. **Reward design:** Accuracy (primary) + turn count minimization (latency proxy) + hallucination penalty. Trained on a single H100 for ~$80, completed in one day. **Beats o3 in accuracy, turn count, and hallucination rate.** Directly illustrates the course's Lesson 4 (GRPO implementation). [[raw/articles/2025-04-29_corbt_art-e-mail-agent]]
- **[Reward Hacking 101](https://corbt.com/posts/reward-hacking)** (2025-06-06) — Introductory article on reward hacking, explained across three levels: nature (bees stealing nectar), organizations (the Cobra effect), and AI models (Sonnet 3.7 editing tests, Gemini 2.5 Pro ignoring errors, GPT-4o sycophancy). **Two OpenPipe case studies:** HN title optimization converging on "Google lays off 80%", NYT Connections solver exploiting a validation bug. **Key lesson:** Trace logging is the key to identifying hacks; simple fixes are often effective. Supplementary material for the course's evaluation/reward design lectures. [[raw/articles/2025-06-06_corbt_reward-hacking-101]]
- **[RULER: Easy Mode for RL Rewards](https://corbt.com/posts/ruler)** (2025-07-11) — RULER (Relative Universal LLM-Elicited Rewards) announcement. A universal reward function requiring no labeled data, no hand-written reward functions, and no human feedback. **Core insight:** Comparing multiple solutions side-by-side is easier than individual scoring; GRPO only needs relative scores within groups. **Benchmarks:** Qwen 2.5 + RULER beats o3 on 4/4 tasks and beats hand-written reward functions on 3/4 tasks. Technology that directly complements the course's Lesson 3 (evals) and Lesson 4 (GRPO). [[raw/articles/2025-07-11_corbt_ruler-easy-mode-rl-rewards]]
