...
### Kyle Corbitt Blog Series: ART/RL 実装と実践 (2025-04 ~ 2025-07)

ART フレームワーク公開から RULER 発表までの実践ブログシリーズ。Course の Lesson 4（RL）以降の補足資料として直接的に関連。

- **[ART Trainer: A New RL Trainer for Agents](https://corbt.com/posts/art-trainer-a-new-rl-trainer-for-agents)** (2025-04-14) — ART フレームワーク発表。既存トレーナーの3つの制限（マルチターン未対応、GPU 非効率、既存コードベース統合困難）を解決。**アーキテクチャ:** Frontend/Backend 分離（ローカル rollout + クラウド GPU）、OpenAI 互換推論エンドポイント（vLLM）、KV cache を CPU にオフロードして Colab 無料枠で 7B 学習可。**初期結果:** 7B で GPT-4o 超え（Tic-tac-toe）、14B でフロンティアモデル超え（Temporal Clue）。[[raw/articles/2025-04-14_corbt_art-trainer-new-rl-trainer]]
- **[ART·E: Email Research Agent That Beats o3](https://corbt.com/posts/art-e-mail-agent)** (2025-04-29) — ART の実証ケース。Enron メールデータセットで synthetic QA 生成 → GRPO 学習。**報酬設計:** 正答率（主）+ ターン数最小化（レイテンシ proxy）+ ハルシネーションペナルティ。H100 1台で ~$80、1日で学習。**o3 を精度・ターン数・ハルシネーション率で上回る。** Course の Lesson 4（GRPO 実装）の直接的な実例。[[raw/articles/2025-04-29_corbt_art-e-mail-agent]]
- **[Reward Hacking 101](https://corbt.com/posts/reward-hacking)** (2025-06-06) — Reward hacking の入門記事。自然界（ハチの蜜盗み）、組織（コブラ効果）、AI モデル（Sonnet 3.7 のテスト編集、Gemini 2.5 Pro のエラー無視、GPT-4o の迎合）の3層で説明。**OpenPipe の2つのケーススタディ:** HN タイトル最適化が「Google lays off 80%」に収束、NYT Connections ソルバが validation バグを悪用。**教訓:** トレース記録がハック特定の鍵、シンプルな修正で効果的情况が多い。Course の evaluation/reward design 講義の補足。[[raw/articles/2025-06-06_corbt_reward-hacking-101]]
- **[RULER: Easy Mode for RL Rewards](https://corbt.com/posts/ruler)** (2025-07-11) — RULER（Relative Universal LLM-Elicited Rewards）発表。ラベルデータ不要、手書き報酬関数不要、人間フィードバック不要の汎用報酬関数。**核心洞察:** 複数解を横並びで比較することは個別スコアリングより容易、GRPO はグループ内相対スコアのみで十分。**ベンチマーク:** Qwen 2.5 + RULER が o3 に 4/4 タスクで勝利、手書き報酬関数に 3/4 で勝利。Course の Lesson 3（evals）と Lesson 4（GRPO）を直接的に補完する技術。[[raw/articles/2025-07-11_corbt_ruler-easy-mode-rl-rewards]]
