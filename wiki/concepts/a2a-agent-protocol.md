...
**Decision heuristic:** Use MCP when you need to give an agent access to databases, APIs, or filesystem tools. Use A2A when you need agents to delegate work to each other across organizational or vendor boundaries. They are often used together in production architectures: MCP handles tool integration, A2A handles cross-agent orchestration. See [[concepts/multi-agents/agent-communication-protocols]] for a detailed comparison.
