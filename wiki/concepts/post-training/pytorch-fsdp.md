...
related:
  ...
  - concepts/post-training/accelerate
...
| Pipeline parallelism | No | No | Yes |
| Complexity | Low | Medium | High |
| ZeRO stages | N/A | 2, 3 | 1, 2, 3, 3‑offload, Infinity |
| Autotuning | Manual | Manual | `ds_report` + auto‑tuning |
...
- Model fits in GPU memory at bf16/fp16 after sharding
...
- You need ZeRO-Stage-1 or 2 (FSDP only offers ZeRO-2/3 equivalents)

> **Zach Mueller (Accelerate lead, Jan 2024):** "Does it all fit in memory? Use FSDP. If you have to do a bit of offloading, might look at DeepSpeed. Be that ZeRO-3 or ZeRO-2 and see sort of what optimizations could go there."
...
FSDP is deeply integrated with Hugging Face's training stack via [[concepts/post-training/accelerate]]:
...
- [[entities/zach-mueller]] — Accelerate lead, FSDP expert
...
- Phil Schmid, "FSDP + Q-LoRA + Llama 3"
...
- Zach Mueller Office Hours (Jan 2024) — FSDP vs DeepSpeed practical guidance
