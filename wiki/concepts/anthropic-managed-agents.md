...
Managed Agentsの4つの新機能がGA/Research Previewとしてリリースされた。詳細は[[concepts/claude-managed-agents]]を参照。

1. **Multi-Agent Orchestration (GA)** — Coordinator agentが最大20の専門sub-agentを管理。Shared filesystem + isolated context windows。25並列スレッドまで。
2. **Outcomes Loop (GA)** — ルーブリック駆動の自己改善サイクル。独立したGrader agentが評価→フィードバックループを回す（最大20イテレーション）。
3. **Dreams (Research Preview)** — 過去セッションを振り返ってMemory Storeを最適化（重複除去、矛盾解消、パターン抽出）。非破壊的。
4. **Webhooks (GA)** — プッシュベースの状態変更通知。`whsec_`署名検証、ペイロードは軽量（event type + idのみ）。
